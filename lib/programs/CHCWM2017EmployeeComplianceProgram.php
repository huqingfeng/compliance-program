...
class CHCWM2017EmployeeCompleteScreeningComplianceView extends CompleteScreeningComplianceView
{
...
class CHCWM2017EmployeeWatchVideosComplianceView extends PlaceHolderComplianceView
{
...
class CHCWM2017EmployeeComplianceProgram extends ComplianceProgram
{
...
//            $data['Has Biometric Override'] = $status->getAttribute('has_biometric_override') ? 'Yes' : 'No';
    ...
    {
        return new CHCWM2017EmployeeWMS2Printer();
    }
    ...
    public function loadGroups()
    {
        ...
        $screeningView = new CHCWM2017EmployeeCompleteScreeningComplianceView($screeningStartDate, $screeningEndDate);
        ...
        $watchVideos = new CHCWM2017EmployeeWatchVideosComplianceView(ComplianceStatus::NOT_COMPLIANT);
...
//        $watchVideos->addLink(new Link('Watch Video Now', sprintf('/compliance_programs/localAction?id=%s&local_action=watch_videos', CHCWM2017EmployeeComplianceProgram::CHCWM_2017_EMPLOYEE_RECORD_ID), false, '_blank'));
        ...
        $coachingOverall->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            $notRequired = SelectQuery::create()
                ...
                ->andWhere('compliance_program_record_id = ?', array(CHCWM2017EmployeeComplianceProgram::CHCWM_2017_EMPLOYEE_RECORD_ID))
        ...
        $spouseParticipation->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            ...
            if(!$spouseUser || (isset($this->options['in_spouse']) && $this->options['in_spouse']))  {
                ...
                $program = ComplianceProgramRecordTable::getInstance()->find(CHCWM2017SpouseComplianceProgram::CHCWM_2017_SPOUSE_RECORD_ID)->getComplianceProgram(array('in_employee' => true));
                ...
                $status->setAttribute('spouse_watch_videos_status', $watchVideosStatus->getStatus());
                $status->setAttribute('spouse_watch_videos_comment', $watchVideosStatus->getComment());
...
//        $biometricsGroup->addComplianceView($smokingView);
    ...
    public function evaluateAndStoreOverallStatus(ComplianceProgramStatus $status)
    {
        ...
        if(!isset($this->options['in_spouse']) || !$this->options['in_spouse']) {
            $priorStatus = $status->getComplianceProgram()->getPriorYearBiometricsComplianceViewStatuses($status->getUser(), '2015-01-01', '2015-01-31');
...
class CHCWM2017EmployeeWMS2Printer implements ComplianceProgramReportPrinter
{
                        ...
                        <?php if($viewStatus->getComplianceView()->getName() == 'spouse_participation') : ?>
                            <?php if($viewStatus->getAttribute('has_spouse')) : ?>
                                ...
                                <tr class="view-spouse_participation_hra">
                                    <td class="name">
                                        <span style="padding-left: 20px;">• Spouse Health Risk Assessment (HRA) questionnaire</span>
                                    </td>
                                ...
                                <tr class="view-spouse_participation_screening">
                                    <td class="name">
                                        <span style="padding-left: 20px;">• Spouse Wellness Screening</span>
                                    </td>
                                ...
                                <tr class="view-spouse_participation_screening">
                                    <td class="name">
                                        <span style="padding-left: 20px;">• Spouse Annual Medical Physical</span>
                                    </td>
                                ...
                                <tr class="view-spouse_participation_screening">
                                    <td class="name">
                                        <span style="padding-left: 20px;">• Spouse Annual Dental Checkup</span>
                                    </td>
                                ...
                                <tr class="view-spouse_participation_screening">
                                    <td class="name">
                                        <span style="padding-left: 20px;">• Spouse Watch Priority Rewards Video</span>
                                    </td>
                                    <td class="points <?php echo $classForStatus($viewStatus->getAttribute('spouse_watch_videos_status')) ?>"
                                        <?php echo $classForStatus($viewStatus->getAttribute('spouse_watch_videos_status')) == 'info' ? 'style="background-color: #CCC;"' : ''?>>
                                        <?php echo $textForSpouseStatus($viewStatus->getAttribute('spouse_watch_videos_status'), $viewStatus->getAttribute('spouse_watch_videos_comment')) ?>
                                    </td>
                                    <td>4/30/2017</td>
                                    <td class="links text-center">
                                    </td>
