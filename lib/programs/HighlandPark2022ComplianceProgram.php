...
class HighlandPark2022HomePageComplianceProgram extends ComplianceProgram
{
    const SCHEDULE_BUTTON_OVERRIDE_TYPE = 'highland_park_schedule_button_override_2022';
    ...
    public function loadGroups()
    {
        ...
        $appointmentView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use($appointmentView) {
            if($user->getNewestDataRecord(HighlandPark2022HomePageComplianceProgram::SCHEDULE_BUTTON_OVERRIDE_TYPE, true)->clicked) {
        ...
        $coachingView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            ...
            $track = $user->getGroupValueFromTypeName('Advocate Track 2022', '0-1 Risk Factors');

            if(isset($coachingViewStatus)) {
                if($coachingViewStatus->getStatus() == ComplianceViewStatus::COMPLIANT) {
                    $text .= 'Coaching Not Required or Completed';
                    if($track == '2 Risk Factors' || $track == '3+ Risk Factors') {
                        $text .= '<div style="font-size: 1.1em; margin-top: 65px;"><a href="/content/1051">Register Here</a></div>';
                    }
                    ...
                    if($track == '2 Risk Factors' || $track == '3+ Risk Factors') {
                        $text .= '<div style="font-size: 1.1em; margin-top: 90px;"><a href="/content/1051">Register Here</a></div>';
                    }
                ...
                if($track == '2 Risk Factors' || $track == '3+ Risk Factors') {
                    $text .= '<div style="font-size: 1.1em; margin-top: 150px;"><a href="/content/1051">Register Here</a></div>';
                }
            }

            $status->getComplianceView()->setAttribute('text', $text);
        });
        ...
        $winView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            ...
            if($activeProgramRecord->id == 1629) {
...
class HighlandPark2022ComplianceProgram extends ComplianceProgram
{
    ...
    public function loadEvaluators()
    {
        ...
        $mine->setMaximumNumberOfPoints(475);
        ...
        $spouse->setMaximumNumberOfPoints(740);
    ...
    public function loadGroups()
    {
        ...
        $wellnessExam->setName('physical_exam');
        $wellnessExam->setReportName('Annual Physical Exam');
        $wellnessExam->setMaximumNumberOfPoints(20);
        $wellnessExam->setAllowPointsOverride(true);
        $wellnessExam->addLink(new Link('Learn More', '#popover_physical_exam'));
        ...
        $lessons->setReportName('eLearning Video (5 points per each; up to 6 per year)');
        ...
        $fitnessTrainings->setName('learn_earns');
        $fitnessTrainings->setReportName('Learn & Earns (15 points per each; up to 7 per year)');
        $fitnessTrainings->setMaximumNumberOfPoints(105);
        $fitnessTrainings->setAllowPointsOverride(true);
//        $fitnessTrainings->addLink(new Link('Learn More', '#popover_learn_earns'));
        ...
        $useCenter->setReportName('Fire/Police Fitness Center Gym Use');
        ...
        $homeGym = new PlaceHolderComplianceView(null, 0);
        $homeGym->setName('home_gym');
        $homeGym->setReportName('Home Gym Use');
        $homeGym->setMaximumNumberOfPoints(20);
        $homeGym->setAllowPointsOverride(true);
        $homeGym->addLink(new Link('Learn More', '#popover_home_gym'));
        $homeGym->setAttribute('link_popover', 'Employees or spouses who use the Fitness Center located at the Fire Station (Fire No.33) or Police employees who use the Police Fitness Center located in the Police Department, or the Rec Center Track will receive 2 points per visit; the maximum points an employee or spouse can receive per program year is 20 points.
        <br /><br />Employees or spouses will receive 2 points per visit (police station is only available to Police employees);  sign in is required to receive points.  Maximum points is 20 points per program year.  Individuals must sign a City consent form prior to using any fitness facilities.  ');
        //$useCenter->setAttribute('report_name_link', '/content/1094#2aFitnessCtr');
        $phase2->addComplianceView($homeGym);

        $track = new PlaceHolderComplianceView(null, 0);
        $track->setName('track');
        $track->setReportName('Rec Center Track Use');
        $track->setMaximumNumberOfPoints(20);
        $track->setAllowPointsOverride(true);
        $track->addLink(new Link('Learn More', '#popover_track'));
        $track->setAttribute('link_popover', 'Employees or spouses who use the Fitness Center located at the Fire Station (Fire No.33) or Police employees who use the Police Fitness Center located in the Police Department, or the Rec Center Track will receive 2 points per visit; the maximum points an employee or spouse can receive per program year is 20 points.
        <br /><br />Employees or spouses will receive 2 points per visit (police station is only available to Police employees);  sign in is required to receive points.  Maximum points is 20 points per program year.  Individuals must sign a City consent form prior to using any fitness facilities.  ');
        //$useCenter->setAttribute('report_name_link', '/content/1094#2aFitnessCtr');
        $phase2->addComplianceView($track);
        ...
        $runWalk->setReportName('Organized Run/Walk (5 points per event; up to 2 per year)');
        ...
        $wellnessFair->setReportName('Wellness Fair (Stay for 2 sessions)');
        $wellnessFair->setMaximumNumberOfPoints(25);
        $wellnessFair->setAllowPointsOverride(true);
        $phase2->addComplianceView($wellnessFair);
        ...
        $one->setAttribute('link_popover', '<span style="font-weight: bold;font-size: 10pt;">Go H2O Challenge</span> <br /><br />December 2021<br />');
        ...
        $two->setAttribute('link_popover', '<span style="font-weight: bold;font-size: 10pt;">Be Positive Challenge</span><br /><br />February 2022<br />');
        ...
        $three->setAttribute('link_popover', '<span style="font-weight: bold;font-size: 10pt;">Step Challenge</span><br /><br />June 2022<br />');
        ...
        $four->setAttribute('link_popover', '<span style="font-weight: bold;font-size: 10pt;">Activity Challenge</span><br /><br />September 2022<br />');
    ...
    public function getAdminProgramReportPrinter()
    {
        ...
        $printer->addEndStatusFieldCallBack('Track', function(ComplianceProgramStatus $status) {
            $user = $status->getUser();

            return $this->getTrack($user);
        });
    ...
    {
        return new HighlandPark2022ComplianceProgramReportPrinter();
    }
...
class HighlandPark2022ComplianceProgramReportPrinter extends BasicComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        $this->setShowLegend(true);
        $this->pageHeading = '2022 WIN (Wellness Initiative Program)';
        ...
        <script type="text/javascript">
            $(function() {
                ...
                $('tr.view-mine').prev('tr.headerRow').find('td').last().html(
                    'Minimum Points Needed for Incentive By 10/31/2022'
                );
                ...
                $('.view-coaching').children(':eq(0)').html('<strong>C</strong>. Health Coaching: Complete 3 Sessions (Deadline September 30th, 2022)');
                ...
                $('.view-coaching').children(':eq(0)').html('<strong>C</strong>. Health Coaching: Complete 6 Sessions (Deadline August 12th, 2022)');
                $('.view-coaching').children('.result').html('<?php echo $coachingStatus->getPoints() ?>');
                $('.view-coaching').children('.links').html('To schedule an appointment, send email to <a href="mailto:AMG-AAWCallCenter@advocatehealth.com">AMG-AAWCallCenter@advocatehealth.com</a> OR Call the Advocate Aurora Contact Center at 800.761.5856');
                <?php else : ?>
