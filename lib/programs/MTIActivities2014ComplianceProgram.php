...
class MTIActivities2014WeeklyLogComplianceView extends CompleteActivityComplianceView
{
    ...
    public function __construct($questionId, $pointsPer, $divisor)
    {
        $this->id = 416;

        parent::__construct('2013-01-01', '2015-12-31');
...
class MTIActivities2014ComplianceProgram extends ComplianceProgram
{
    const MTI_2014_ACTIVITY_START_DATE = '2015-01-01';
    ...
    protected function getActivityView($name, $activityId, $points, $reportName = null)
    {
        $view = new CompleteArbitraryActivityComplianceView(
            MTIActivities2014ComplianceProgram::MTI_2014_ACTIVITY_START_DATE,
            $this->getEndDate(),
            $activityId,
            $points
        );
    ...
    {
      $view = new MTIActivities2014WeeklyLogComplianceView($questionId, $pointsPer, $divisor);
    ...
    {
        $printer = new MTIActivities2014ComplianceProgramPrinter();
    ...
    public function loadGroups()
    {
        ...
        $fluTetView = new ComplyWithMultipleHraQuestionsComplianceView(MTIActivities2014ComplianceProgram::MTI_2014_ACTIVITY_START_DATE, $endDate, array(30, 31));
        ...
        $learningView = new CompleteELearningGroupSet(MTIActivities2014ComplianceProgram::MTI_2014_ACTIVITY_START_DATE, $endDate, 'mti_prevention');
        ...
        $learningView = new CompleteELearningGroupSet(MTIActivities2014ComplianceProgram::MTI_2014_ACTIVITY_START_DATE, $endDate, 'mti_fitness');
        $learningView->setReportName('Complete an e-learning module');
        $learningView->setNumberRequired(1);
        $learningView->setComplianceStatusPointMapper(new ComplianceStatusPointMapper(5, 0, 0, 0));
        ...
        $learningView = new CompleteELearningGroupSet(MTIActivities2014ComplianceProgram::MTI_2014_ACTIVITY_START_DATE, $endDate, 'light_activities');
        $learningView->setReportName('Complete an e-learning module');
        $learningView->setNumberRequired(1);
        $learningView->setComplianceStatusPointMapper(new ComplianceStatusPointMapper(5, 0, 0, 0));
        ...
        $learningView = new CompleteELearningGroupSet(MTIActivities2014ComplianceProgram::MTI_2014_ACTIVITY_START_DATE, $endDate, 'mti_stress');
        $learningView->setReportName('Complete an e-learning module');
        $learningView->setNumberRequired(1);
        $learningView->setComplianceStatusPointMapper(new ComplianceStatusPointMapper(5, 0, 0, 0));
...
class MTIActivities2014ComplianceProgramPrinter extends CHPComplianceProgramReportPrinter
{
        ...
        <script type="text/javascript">
            $(function() {
                $.get('/compliance_programs?id=388', function(fullPage) {
                    ...
                    $('#biometric_points').html(bioPoints);

                    $('#total_points').html('' + (bioPoints + <?php echo $status->getPoints() ?>));
                });
    ...
    protected function printCustomRows(ComplianceProgramStatus $status)
    {
        ?>
        <tr class="headerRow">
            <th colspan="2">My Wellness Activity Points (370 possible)-</th>
            <td id="activity_points"><?php echo $status->getPoints() ?></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="headerRow">
            <th colspan="2">My Biometric Points (80 possible)-</th>
            <td id="biometric_points"></td>
            <td></td>
            <td></td>
        </tr>
        <tr class="headerRow">
            <th colspan="2">My Total Points (450 possible)-</th>
