...
class MacAllisterMachinery2020ComplianceProgram extends ComplianceProgram
{
    ...
    {
        return new MacAllisterMachinery2020ComplianceProgramReportPrinter();
    }
...
//        $coreGroup->setPointsRequiredForCompliance(50);
        ...
        $screeningView->setAttribute('goal', '11/2/2020');
        ...
        $tobaccoView = new ComplyWithTobaccoUseScreeningTestComplianceView($programStart, $programEnd);
        ...
        $tobaccoView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use($programStart, $programEnd) {
            ...
            $elearningView = new CompleteELearningGroupSet('2020-02-01', $programEnd, 'tobacco_2019');
            ...
            } elseif($status->getComment() == 'Non-User') {
        ...
        $bmiView->addResultMapping('Declined', ComplianceStatus::NOT_COMPLIANT, 'Declined');
        $bmiView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use($programStart, $programEnd) {
            ...
            if($status->getComment() == 'Test Not Taken' || $status->getComment() == 'No Screening' || $status->getComment() == '') {
                $status->setStatus(ComplianceStatus::NOT_COMPLIANT);
                $healthRiskPoints = '1';
            } elseif($status->getComment() == 'Declined') {
                $status->setStatus(ComplianceStatus::NOT_COMPLIANT);
                $healthRiskPoints = '1';
            } else {
                if($status->getStatus() == ComplianceStatus::COMPLIANT) {
        ...
        $bpView->addSystolicResultMapping('Declined', ComplianceStatus::NOT_COMPLIANT, 'Declined');
        $bpView->addDiastolicResultMapping('Declined', ComplianceStatus::NOT_COMPLIANT, 'Declined');
        $bpView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use($programStart, $programEnd) {
            ...
            if($status->getComment() == 'Test Not Taken' || $status->getComment() == 'No Screening' || $status->getComment() == '') {
                $status->setStatus(ComplianceStatus::NOT_COMPLIANT);
                $healthRiskPoints = '1';
            } elseif($status->getComment() == 'Declined' || $status->getComment() == 'Declined/Declined') {
                $status->setStatus(ComplianceStatus::NOT_COMPLIANT);
                $healthRiskPoints = '1';
            } else {
                if($status->getStatus() == ComplianceStatus::COMPLIANT) {
        ...
        $tcView->addResultMapping('Declined', ComplianceStatus::NOT_COMPLIANT, 'Declined');
        ...
        $gluView->addResultMapping('Declined', ComplianceStatus::NOT_COMPLIANT, 'Declined');
...
class MacAllisterMachinery2020ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
                         ...
                         <?php else : ?>
                            <td colspan="3" style="background-color: #ffb3b3">
                                <p style="margin:10px 20px;">
                                    ...
                                    <a href="/resources/10571/2020_MacAllister Machinery_AQF_070520.pdf">Click here to download the AQF.</a>
                                </p>
                                <p>
                                    AQF's are due NO LATER THAN 11/30/20.
                                </p>
                            </td>
                            <td style="background-color: #ffb3b3"><?php echo $status->getAttribute('health_risk_points') ?></td>
                         <?php endif ?>
