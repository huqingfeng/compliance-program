...
define('BEACON_FIVE_FOR_FIVE_CAMPAIGN_2018', 'beacon_five_for_five_campaign_2018');
define('BEACON_FIVE_FOR_FIVE_CAMPAIGN_2018_START_DATE', '2018-07-09');
define('BEACON_FIVE_FOR_FIVE_CAMPAIGN_2018_END_DATE', '2018-08-12');

class Beacon5For5Campaign2018WeeklyPointsComplianceView extends ComplianceView
{
    ...
    {
        $record = $user->getNewestDataRecord(BEACON_FIVE_FOR_FIVE_CAMPAIGN_2018, true);
        $points = BeaconFiveForFiveCampaign2018CommitmentForm::getPoints($record);
...
class Beacon5For5Campaign2018AdminPrinter extends BasicComplianceProgramAdminReportPrinter
{
...
class Beacon5For5Campaign2018ComplianceProgram extends ComplianceProgram
{
    public function getAdminProgramReportPrinter()
    {
        $record = $this->getComplianceProgramRecord();

        return new Beacon5For5Campaign2018AdminPrinter($record);
    }

    public function getTeamDashboardPrinter()
    {
        return new Beacon5For5Campaign2018TeamDashboardPrinter();
    }

    public function getTeamData(sfActions $actions)
    {
        $activeProgramRecord = ComplianceProgramRecordTable::getInstance()->find(1360);

        $teams = $activeProgramRecord->getTeams();

        $allUsersPoints = BeaconFiveForFiveCampaign2018CommitmentForm::getAllUsersPoints($actions);
    ...
    {
        return new Beacon5For5Campaign2018BuddyDashboardPrinter();
    }

    public function getProgramReportPrinter($preferredPrinter = null)
    {
        return new Beacon5For5Campaign2018ProgramReportPrinter();
    }
    ...
    public function loadGroups()
    {
        $this->options = array(
                ...
                'team_create_end_date'      => '2018-08-01',
                'team_leaderboard'           => true,
                'force_spouse_with_employee'  => true,
                'force_spouse_with_employee_excluded_spouses'   => array(
                    2772315, 2796088, 2770375, 2796313, 3481686, 2766530, 2930570, 2795621, 2795841, 3537460, 2794710,
                    2773569, 2770423, 2766700, 3485424, 2773896, 2775045, 2971454, 2770233, 2766814, 2869459, 3003412,
                    2766257, 2767330, 2768333, 2766217, 2915885, 2770979, 2765472, 2869832, 2769379, 2765640, 2769321,
                    2775089, 2767464, 2766741, 2771017, 2766599, 2765285, 2770132, 2930663, 2767292, 2767416, 2769206,
                    2781485, 2769848, 2768727, 2696733, 2887676, 2766465, 2855445, 3499712, 2769842, 2770178, 2893787,
                    2806954, 3068347, 2769330, 2997828, 2767691, 2795692, 2766323, 2775446, 2939534, 2769895, 2769331,
                    2846619, 3485439, 3065011, 2878313, 3141394, 3525723, 2769108, 2776442, 2887643, 2765488, 2766942,
                    2767225, 2794640, 2796209, 3488079, 2766528, 2769559, 2776480, 3480807, 2849972, 2887769, 2768725,
                    2768746, 3003780, 2796838, 2771405, 3494729, 2797015, 2767101, 2769877, 2797034, 2772946, 2767311,
                    3534854, 2770552, 3002666, 2795689, 2769267
                ),
                'points_label'               => 'points',
                'total_steps'               => false,
            ) + $this->options;
        ...
        $week1View = new Beacon5For5Campaign2018WeeklyPointsComplianceView('week1');
        $week1View->setReportName('Week 1 Points');
        $week1View->setName('week1_points');
        $week1View->addLink(new Link('Log fruits & vegetables', '/compliance_programs/localAction?id=1360&local_action=5for5_dashboard'));
        $operations->addComplianceView($week1View);

        $week2View = new Beacon5For5Campaign2018WeeklyPointsComplianceView('week2');
        $week2View->setReportName('Week 2 Points');
        $week2View->setName('week2_points');
        $week2View->addLink(new Link('Log fruits & vegetables', '/compliance_programs/localAction?id=1360&local_action=5for5_dashboard'));
        $operations->addComplianceView($week2View);

        $week3View = new Beacon5For5Campaign2018WeeklyPointsComplianceView('week3');
        $week3View->setReportName('Week 3 Points');
        $week3View->setName('week3_points');
        $week3View->addLink(new Link('Log fruits & vegetables', '/compliance_programs/localAction?id=1360&local_action=5for5_dashboard'));
        $operations->addComplianceView($week3View);

        $week4View = new Beacon5For5Campaign2018WeeklyPointsComplianceView('week4');
        $week4View->setReportName('Week 4 Points');
        $week4View->setName('week4_points');
        $week4View->addLink(new Link('Log fruits & vegetables', '/compliance_programs/localAction?id=1360&local_action=5for5_dashboard'));
        $operations->addComplianceView($week4View);

        $week5View = new Beacon5For5Campaign2018WeeklyPointsComplianceView('week5');
        $week5View->setReportName('Week 5 Points');
        $week5View->setName('week5_points');
        $week5View->addLink(new Link('Log fruits & vegetables', '/compliance_programs/localAction?id=1360&local_action=5for5_dashboard'));
        ...
        <script type="text/javascript">
            $(function() {
                $('#compliance_tabs').before(
                    '<img src="/resources/10105/beacon_5for5_banner.jpg" />' +
...
EOT
    ...
    public function execute5For5Dashboard(sfActions $actions)
    {
        ...
        $form = new BeaconFiveForFiveCampaign2018CommitmentForm(array(), array('user' => $user));

        $record = $user->getNewestDataRecord(BEACON_FIVE_FOR_FIVE_CAMPAIGN_2018, true);

        $points = BeaconFiveForFiveCampaign2018CommitmentForm::getPoints($record);
        $clientPoints = BeaconFiveForFiveCampaign2018CommitmentForm::getClientPoints($actions);
        ...
        <div id="five-for-five">
            <div id="header_image"><img src="/resources/10105/beacon_5for5_banner.jpg" style="width:99%;"/></div>
                ...
                <div class="tab-pane active" id="record_your_food">
                    <div>
                        <div style=" font-size: 11pt;">
                            <br /><a href="/compliance_programs?id=1360">Back to my dashboard</a>
                        </div>


                        <div style="width: 36%; height: 300px;float: left; color: #ffffff; background-color: #8dc63f;border-radius: 25px; margin: 30px 30px;">
                            <div style="width: 80%; margin: 30px auto;font-size: 11pt; height:100px; "><div style="font-size: 15pt; margin-bottom:10px;">Challenge 1:</div>
                        ...
                        <div style="width: 36%; height: 300px; float: left; color: #ffffff; background-color: #652d95;border-radius: 25px;margin: 30px 30px;">
                            <div style="width: 80%; margin: 30px auto;font-size: 11pt; height:100px; "><div style="font-size: 15pt; margin-bottom:10px;">Challenge 2:</div>
                    ...
                    <?php echo $form->renderFormTag('/compliance_programs/localAction?id=1360&local_action=save_your_fruits') ?>
                    <?php echo $form->renderGlobalErrors() ?>
                    <?php echo $form->renderHiddenFields() ?>
                    <div  style="clear:both;">
                        <?php foreach(BeaconFiveForFiveCampaign2018CommitmentForm::getWeeks($user) as $week => $weekName) :?>
                            <?php $startEndDates = BeaconFiveForFiveCampaign2018CommitmentForm::getWeekStartEndDates(); ?>
                            ...
                            <div style="padding-top:15px;" id="<?php echo $week ?>">
                                ...
                                <div class="week_content">
                                    ...
                                    <?php foreach(BeaconFiveForFiveCampaign2018CommitmentForm::getDays() as $day => $dayName) : ?>
                                        <span class="label label-success" style="line-height:20px; padding-top: 6px;"><?php echo $dayName ?></span>

                                        <?php foreach(BeaconFiveForFiveCampaign2018CommitmentForm::getFruitTypes() as $fruitType) :?>
    ...
    public function executeSaveYourFruits($actions)
    {
        ...
        $weeks = array_keys(BeaconFiveForFiveCampaign2018CommitmentForm::getWeeks($user));
        $weekStartEndDates = BeaconFiveForFiveCampaign2018CommitmentForm::getWeekStartEndDates();
                    ...
                    if(!empty($value)) {
                        foreach($weekStartEndDates as $week => $startEndDate) {
                            if(strpos($field, $week) !== false
                                ...
                                $actions->redirect('/compliance_programs/localAction?id=1360&local_action=5for5_dashboard#record_your_food');
                            }
                        }
        ...
        $actions->redirect('/compliance_programs/localAction?id=1360&local_action=5for5_dashboard');
    }
...
class Beacon5For5Campaign2018ProgramReportPrinter implements ComplianceProgramReportPrinter
{
...
class Beacon5For5Campaign2018TeamDashboardPrinter extends Beacon5For5Campaign2018BuddyAndTeamDashboardPrinter implements TeamDashboardPrinter
{
...
class Beacon5For5Campaign2018BuddyDashboardPrinter extends Beacon5For5Campaign2018BuddyAndTeamDashboardPrinter implements BuddyDashboardPrinter
{
...
abstract class Beacon5For5Campaign2018BuddyAndTeamDashboardPrinter
{
...
class BeaconFiveForFiveCampaign2018CommitmentForm extends BaseForm
{
    public function configure()
    {
        ...
        if($record->exists()){
            foreach($record->getAllDataFields() as $field) {
                if(BEACON_FIVE_FOR_FIVE_CAMPAIGN_2018_START_DATE <= date('Y-m-d', strtotime($field->getField('creation_date')))
                    && date('Y-m-d', strtotime($field->getField('creation_date'))) <= BEACON_FIVE_FOR_FIVE_CAMPAIGN_2018_END_DATE) {
        ...
        $weeks = array_keys(BeaconFiveForFiveCampaign2018CommitmentForm::getWeeks($user));
        $days = array_keys(BeaconFiveForFiveCampaign2018CommitmentForm::getDays());
        $fruitTypes = BeaconFiveForFiveCampaign2018CommitmentForm::getFruitTypes();


        $widgets = array();
        $validators = array();

        $fruitChoices = BeaconFiveForFiveCampaign2018CommitmentForm::getFruitChoices();
    ...
    public static function getAllUsersPoints($actions)
    {
        ...
        $records = UserDataRecord::getRecordsForAllUsers(BEACON_FIVE_FOR_FIVE_CAMPAIGN_2018, $client, true);
