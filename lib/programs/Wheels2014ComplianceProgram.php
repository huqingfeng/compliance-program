...
class Wheels2014ComplianceProgram extends ComplianceProgram
{
    const POINTS = 80;
    ...
    public function getAdminProgramReportPrinter()
    {
        ...
        $printer->addMultipleStatusFieldsCallback(function(ComplianceProgramStatus $status) {
            $oldPoints = $status->getComplianceViewGroupStatus('2013_points')->getPoints();
            $newPoints = $status->getComplianceViewGroupStatus('points')->getPoints();

            return array(
                '2013_points'                    => $oldPoints,
                '2014_points'                    => $newPoints,
                '2014_2013_change'               => bcsub($newPoints, $oldPoints, 1),
                Wheels2014ComplianceProgram::POINTS.'+ in 2014, or 3+ improvement' => $status->getComplianceProgram()->isCompliantFor1C($oldPoints, $newPoints)
            );
        });
    ...
    {
        return $newPoints >= Wheels2014ComplianceProgram::POINTS || ($oldPoints > 0 && $newPoints - $oldPoints >= 3);
    }
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            $printer = new Wheels2014ScreeningTestPrinter();
            $printer->setShowLight(true);
            $printer->setShowPoints(false);

            return $printer;
        } else {
            $printer = new Wheels2014ComplianceProgramReportPrinter();
        }
    ...
    {
        $oldProgramRecord = ComplianceProgramRecordTable::getInstance()->find(282);

        $screeningTestAlias = 'wheels_2014';
        $lastYearsTestAlias = 'wheels_2013';
        ...
        $oldPointsGroup = new ComplianceViewGroup('2013_points', '2013 Numbers');

        $screeningModel = new ComplianceScreeningModel();

        $startDate = $this->getStartDate();
        $endDate = $this->getEndDate();
        $endDateText = date('m/d/Y', strtotime($endDate));
        $coreEndDate = '2015-08-31';
        $coreEndDateText = date('m/d/Y', strtotime($coreEndDate));

        $screeningTestViews = $this->getScreeningTestViews($screeningModel, $screeningTestAlias, $startDate, $endDate, new \DateTime('2014-10-01'), '2014_', null, null);
        $oldScreeningTestViews = $this->getScreeningTestViews($screeningModel, $lastYearsTestAlias, '2013-01-01', '2013-12-31', new \DateTime('2013-11-05'), '', $oldProgramRecord, 'bodyfat');

        $core = new ComplianceViewGroup('core', 'Core actions required by '.$coreEndDateText);

        $scrView = new CompleteScreeningComplianceView($startDate, $coreEndDate);
        $scrView->setReportName('Complete Wellness Screening');
        $scrView->setAttribute('deadline', $coreEndDate);
        ...
        $hpaView->setAttribute('deadline', $coreEndDate);
        ...
        $cotinineView = new EvaluateScreeningTestResultComplianceView($screeningModel, 'cotinine', $startDate, $endDate, $lastYearsTestAlias);
...
class Wheels2014ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        $totalCholesterolStatus = $pointGroupStatus->getComplianceViewStatus('2014_comply_with_screening_test_cholesterol');
        $hdlStatus = $pointGroupStatus->getComplianceViewStatus('2014_comply_with_screening_test_hdl');
        $ldlStatus = $pointGroupStatus->getComplianceViewStatus('2014_comply_with_screening_test_ldl');
        $triglyceridesStatus = $pointGroupStatus->getComplianceViewStatus('2014_comply_with_screening_test_triglycerides');
        $glucoseStatus = $pointGroupStatus->getComplianceViewStatus('2014_comply_with_screening_test_glucose');
        $systolicStatus = $pointGroupStatus->getComplianceViewStatus('2014_comply_with_screening_test_systolic');
        $diastolicStatus = $pointGroupStatus->getComplianceViewStatus('2014_comply_with_screening_test_diastolic');
        $bodyFatBMIStatus = $pointGroupStatus->getComplianceViewStatus('2014_comply_with_best_screening_test_bodyfat_bmi');
        ...
        $oldBiometricPoints = $status->getComplianceViewGroupStatus('2013_points')->getPoints();
        ?>
    <style type="text/css">
    ...
    <p>Please allow at least 5-10 days after your 2014 wellness screening for your results and points to show.</p>
    ...
    <tr>
        <td><a href="/content/1094#1bHPA">C. Get <?php echo Wheels2014ComplianceProgram::POINTS ?> biometric points from your 2014 Wellness Screening; OR; get 3 or more biometric points over last year's total.</a>
        </td>
        <td class="center">
            <?php echo $completeScreeningStatus->getComplianceView()->getAttribute('deadline') ?>
        </td>
    ...
    <tr>
        <td colspan="2" class="left">
                        ...
                        <div style="text-align:right;">A. Total Biometric Points for 2014 = </div>
                        <div style="text-align:right;">B. Total Points from 2013 = </div>
                        <div style="text-align:right;">-------------------------------------------- </div>
                        <div style="text-align:right;">C. Point Increase or (Decrease) in 2014 =</div>
        </td>
        <td class="center">
...
class Wheels2014ScreeningTestPrinter extends ScreeningProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ?>
