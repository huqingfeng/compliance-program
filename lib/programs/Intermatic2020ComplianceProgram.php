...
class Intermatic2020ComplianceProgram extends ComplianceProgram
{
    ...
    {
        return new Intermatic2020ComplianceProgramReportPrinter();
    }
    ...
    public function loadGroups()
    {
        ...
        $hraScore->setPreMapCallback(function(ComplianceViewStatus $status, User $user) {
            $thisYearResult = $status->getComment();

            $view = $status->getComplianceView();

            $lastHraScore = new HraScoreComplianceView('2019-01-01', '2019-12-31', 65);
            ...
            if($thisYearResult && $lastYearResult) {
                ...
                if ($lastYearResult * 1.1 > $thisYearResult) {
                    $isImproved = false;
                }
                ...
                if ($isImproved) {
                    if($status->getStatus() != ComplianceStatus::COMPLIANT) {
                        ...
                        $status->setAttribute('compliant_by_improvement', true);
                    }


                    $status->setAttribute('improved', true);
                }
            }
        });
    ...
    public function evaluateAndStoreOverallStatus(ComplianceProgramStatus $status)
    {
        ...
        $lastEnd = new \DateTime('2019-12-31');
        ...
        $numImproved = $this->checkImprovement($healthGroupStatus, $screeningData);

        if($status->getComplianceViewStatus('hra_score')->getAttribute('improved')) {
            $numImproved++;
        }

        $numCompliant = $this->getNumCompliant($status);

        if($numCompliant >= 4 || $numImproved >= 2) {
            $status->setStatus(ComplianceStatus::COMPLIANT);
        }
    }
    ...
    private function checkImprovement($groupStatus, $screeningData)
    {
        ...
        foreach($groupStatus->getComplianceViewStatuses() as $viewStatus) {
            ...
            if ($screeningData['this'] && $screeningData['last']) {
                ...
                $lastVal = isset($screeningData['last'][$viewName]) ? (float) $screeningData['last'][$viewName] : null;
                $thisVal = isset($screeningData['this'][$viewName]) ? (float) $screeningData['this'][$viewName] : null;
                ...
                if ($isImproved) {
                    if($viewStatus->getStatus() != ComplianceStatus::COMPLIANT) {
                        ...
                        $viewStatus->setAttribute('compliant_by_improvement', true);
                    }


                    $viewStatus->setAttribute('improved', true);
                    $numImproved++;
                }
            }
        }
    ...
    private function getNumCompliant($status)
    {
        ...
        foreach($measureGroupStatus->getComplianceViewStatuses() as $viewStatus) {
            ...
            if($view->getAttribute('screening_view')) {
                if($viewStatus->getStatus() == ComplianceViewStatus::COMPLIANT && !$viewStatus->getAttribute('compliant_by_improvement')) {
...
class Intermatic2020ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        <div class="letter">
            ...
            <p>
                Thank you for participating in the 2020 Wellness Screening. In partnership with Health Maintenance
                 Institute, Intermatic has selected five "Healthy Measures" for you to strive to achieve.
            </p>

            <p>
                As communicated, a medical premium discount will be offered to those employees who achieve at least 4 out
                of the 5 "Healthy Measures" below OR achieve a 10% improvement in any two of the "Healthy Measures"
                from your 2019 screening. If you do not meet the requirements, you may still qualify for the incentive
                by having your doctor complete and sign the Alternate Qualification Form and fax it to Health
                Maintenance Institute at 630-385-0156. If you do not successfully complete either of these options,
                you will not be eligible for the employee medical premium discount this year.
            </p>
            ...
            <?php if($status->getStatus() == ComplianceStatus::COMPLIANT) : ?>
                ...
                <p>
                    CONGRATULATIONS! Based on your results of the 2020 wellness screening, you either fall within the
                ...
                <p>If you have any questions, please contact <a href="mailto:support@hmihealth.com">support@hmihealth.com</a> or call 847-635-6580.</p>
            <?php else : ?>
                <div id="not_compliant_notes">
                    <p class="bund">RESULTS:</p>

                    <p>Based on your results of the 2020 wellness screening, you <span class="bund">DO NOT</span> fall within
                    ...
                    <p>In order to receive the discount, we <strong>must</strong> have documentation from your Physician
                        acknowledging the "Healthy Measure" areas of concern. This
                        <a href="/resources/10516/New_Hire_AQF_Intermatic.pdf">Alternate Qualification Form</a> will need
                        to be completed by your physician, and returned to Health Maintenance Institute. If your completed
                        form is not received by the deadline, you will not qualify for the discount.</p>

                    <p>
                        If you have any questions, please contact <a href="mailto:support@hmihealth.com">support@hmihealth.com</a> or call 847-635-6580.
                    </p>
