...
class Wheels2018ComplianceProgram extends ComplianceProgram
{
    const POINTS = 81;
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            $printer = new Wheels2018ScreeningTestPrinter();
            $printer->setShowLight(true);
            $printer->setShowPoints(false);

            return $printer;
        } else {
            $printer = new Wheels2018ComplianceProgramReportPrinter();
        }
    ...
    public function loadGroups()
    {
        ...
        $pointsGroups = new ComplianceViewGroup('points', 'Optional Health Resources');
        ...
        $hpaView->setReportName('Health Risk Assessment (HRA) Questionnaire');
        $hpaView->setAttribute('deadline', '11/21/2018');
        ...
        $altTobacco = new PlaceHolderComplianceView(ComplianceStatus::NOT_COMPLIANT);
        $altTobacco->setReportName('Participate & Complete Onsite Smoking Cessation Program');
        $altTobacco->setName('alt_tobacco');
        $altTobacco->setAllowPointsOverride(true);
        $alternativeGroup->addComplianceView($altTobacco);

        $altLifestyleHealth = new PlaceHolderComplianceView(ComplianceStatus::NA_COMPLIANT);
        $altLifestyleHealth->setReportName('6 Lifestyle Health Sessions');
        $altLifestyleHealth->setName('alt_lifestyle_health');
        $altLifestyleHealth->setAllowPointsOverride(true);
        $alternativeGroup->addComplianceView($altLifestyleHealth);

        $altHealthActivities = new PlaceHolderComplianceView(ComplianceStatus::NA_COMPLIANT);
        $altHealthActivities->setReportName('2 Health Activities');
        $altHealthActivities->setName('alt_health_activities');
        $altHealthActivities->setAllowPointsOverride(true);
        $alternativeGroup->addComplianceView($altHealthActivities);

        $altEngagement = new PlaceHolderComplianceView(ComplianceStatus::NA_COMPLIANT);
        $altEngagement->setReportName('Three Month Engagement with Onsite Nurse Care Manager');
        $altEngagement->setName('alternative_wellness_activities');
        $altEngagement->setAllowPointsOverride(true);
        $alternativeGroup->addComplianceView($altEngagement);
    ...
    public function evaluateAndStoreOverallStatus(ComplianceProgramStatus $status)
    {
        ...
        $tobaccoStatus = $status->getComplianceViewStatus('cotinine');
        $altTobaccoStatus = $status->getComplianceViewStatus('alt_tobacco');
        $altLifestyleHealthStatus = $status->getComplianceViewStatus('alt_lifestyle_health');
        $altEngagementStatus = $status->getComplianceViewStatus('alternative_wellness_activities');
        $altHealthActivitiesStatus = $status->getComplianceViewStatus('alt_health_activities');

        if($pointsGroupStatus->getPoints() >= 81) {
        } elseif($pointsGroupStatus->getPoints() >= 60) {
            $altLifestyleHealthStatus->setStatus(ComplianceStatus::NOT_COMPLIANT);
            $altHealthActivitiesStatus->setStatus(ComplianceStatus::NOT_COMPLIANT);
        } else {
            if ($altEngagementStatus->getStatus() != ComplianceStatus::COMPLIANT) {
                $altEngagementStatus->setStatus(ComplianceStatus::NOT_COMPLIANT);
            }
            $altHealthActivitiesStatus->setStatus(ComplianceStatus::NOT_COMPLIANT);
        }

        if ($tobaccoStatus->getComment() == "Negative") {
            $altTobaccoStatus->setStatus(ComplianceStatus::COMPLIANT);
        }

        if ($altLifestyleHealthStatus->getPoints() >= 6) {
            $altLifestyleHealthStatus->setStatus(ComplianceStatus::COMPLIANT);
        }

        if ($altHealthActivitiesStatus->getPoints() >= 2) {
            $altHealthActivitiesStatus->setStatus(ComplianceStatus::COMPLIANT);
        }
    }
...
class Wheels2018ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        $nonSmokingStatus = $tobaccoGroupStatus->getComplianceViewStatus('cotinine');
        ...
        <p>Stay tuned for exciting wellness program changes in 2019!</p>
        ...
        <table class="phipTable" border="1">
            ...
            <tr class="headerRow" style="height: 50px;">
                ...
                <td>Optional Health Resources</td>
            </tr>
            <tr>
            ...
            <tr>
                <td>A. <?php echo $nonSmokingStatus->getComplianceView()
                            ->getReportName() ?></td>
                <td class="center">
            ...
            <tr class="headerRow">
                <th colspan="2">4. My HRA Score (from #2 above)</th>
                <td>Totals</td>
                <td colspan="2"></td>
            </tr>
            <tr>
                <td colspan="2" class="left">
                    <div style="font-weight: 600;color:#434343;font-size:10pt;">My Annual Biometric Point Totals for this year</div>
                </td>
                <td class="center">
                    <div style="font-weight: 600;font-size:10pt;text-align:center;"><?php echo $newBiometricPoints ?></div>
                </td>
                <td class="center">
            ...
            <tr class="headerRow">
                <th colspan="2">5. Additional Program Requirements Based on HRA Score and Tobacco Status:</th>
                <td>Status</td>
                <td>Count Completed</td>
                <td></td>
            </tr>

            <?php if($nonSmokingStatus->getComment() != "No Screening"): ?>
            <tr>
                <td colspan="2" class="left">
                    Test Tobacco Free during Fall 2018 screening or participate and complete onsite or approved smoking cessation program/ low dose CT scan for qualified individuals
                </td>
                <td class="center">
                    <div style="text-align:center;"><img src="<?php echo $altTobaccoStatus->getLight(); ?>" class="light"/></div>
                </td>
                <td class="center">
                    Not Applicable
                </td>
            ...
            <?php if ($newBiometricPoints >= 81): ?>
                <tr>
                    <td colspan="2" class="left">
                        Earn 81 Points or Higher
                    </td>
                    <td class="center">
                        <div style="text-align:center;"><img src="/images/lights/greenlight.gif" class="light"></div>
                    </td>
                    <td class="center">
                        Not Applicable
                    </td>
            ...
            <?php elseif ($newBiometricPoints >= 60): ?>
                <tr>
                    <td colspan="2" class="left">
                        Complete 6 Lifestyle Health Session with Onsite Health Coach
                    </td>
                    <td class="center">
                        <div style="text-align:center;">
                            <img src="<?php echo $altLifestyleHealthStatus->getLight(); ?>" class="light"/>
                        </div>
                    </td>
                    <td class="center">
                        <?php echo $altLifestyleHealthStatus->getPoints()?? "0"; ?>
                    </td>
                    <td class="center">
                    </td>
                </tr>
                <tr>
                    <td colspan="2" class="left">
                        2 Health Activities defined by Onsite Health Coach
                    </td>
                    <td class="center">
                        <div style="text-align:center;">
                            <img src="<?php echo $altHealthActivitiesStatus->getLight(); ?>" class="light"/>
                        </div>
                    </td>
                    <td class="center">
                        <?php echo $altHealthActivitiesStatus->getPoints()?? "0"; ?>
                    </td>
                    <td class="center">
                    </td>
                </tr>
            <?php else: ?>
                <tr>
                    <td colspan="2" class="left">
                        Complete Three Month engagement with onsite Nurse Care Manager
                    </td>
                    <td class="center">
                        <div style="text-align:center;">
                            <img src="<?php echo $altEngagementStatus->getLight(); ?>" class="light"/>
                        </div>
                    </td>
                    <td class="center">
                        Not Applicable
...
class Wheels2018ScreeningTestPrinter extends ScreeningProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ?>
