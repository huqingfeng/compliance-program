...
class SandCElectric2020CompleteScreeningComplianceView extends CompleteScreeningComplianceView
{
...
class SandCElectric2020ComplianceProgram extends ComplianceProgram
{
    public function getTrack(User $user)
    {
        if($this->lastTrack && $this->lastTrack['user_id'] == $user->id) {
            ...
            $track = $user->getGroupValueFromTypeName('S&C Track 2020', '0-2 Risk Factors');
    ...
    public function loadGroups()
    {
        ...
        $ampSignup->setAttribute('deadline', '03/31/2020');
        ...
        $screening = new SandCElectric2020CompleteScreeningComplianceView($startDate, $endDate);
        $screening->setComplianceStatusPointMapper(new ComplianceStatusPointMapper(1, 0, 0, 0));
        $screening->setAttribute('deadline', '03/31/2020');
        ...
        $screening->addLink(new Link('Physician Form', '/resources/10482/Form_846_AU_Physician_Screening_10_19.pdf'));
        ...
        $hra->setAttribute('deadline', '03/31/2020');
        $hra->emptyLinks();
        $hra->setName('hra');
        $hra->addLink(new Link('Health Risk Assessment', "/surveys/39"));
        $hra->addLink(new Link('Health Risk Assessment (PDF)', '/resources/10483/Health_Risk_Assessment_2020.pdf'));
        ...
        $hra->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            ...
            if($surveyStatus->getStatus() == ComplianceStatus::COMPLIANT
                && date('Y-m-d', strtotime($surveyStatus->getComment())) >= '2019-10-01')
            {
    ...
    {
        return new SandCElectric2020ComplianceProgramReportPrinter();
    }
    ...
    public function getAdminProgramReportPrinter() {
        ...
        $printer->addCallbackField('Weigh In Screening Date', function (User $user) {
            $data = SandCElectric2020ComplianceProgram::getScreeningData($user, '2019-10-01', '2020-09-30');
        ...
        $printer->addCallbackField('Weight Out Screening Date', function (User $user) {
            $data = SandCElectric2020ComplianceProgram::getScreeningData($user, '2020-10-01', '2020-11-06');
    ...
    public static function getScreeningData(User $user, $startDate = '2019-10-01', $endDate = '2020-09-30')
    {
        $data = ScreeningTable::getInstance()->findOneCompletedForUserBetweenDates($user,
...
class SandCElectric2020ComplianceProgramReportPrinter extends BasicComplianceProgramReportPrinter
{
    ...
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        $this->pageHeading = '2020 Wellness Initiative Program';
