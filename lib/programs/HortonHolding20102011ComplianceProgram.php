...
class HortonGraduateFromCoachingSessionComplianceView extends GraduateFromCoachingSessionComplianceView
{
...
/**
 * These activities have different point values depending upon options
 * selected. Each question answer has the phrase [0-9]+ points in the name
 * so we use a regex to figure out how many points its worth.
 */
class HortonOptionBasedActivity extends CompleteActivityComplianceView
{
...
class HortonWorkshop extends AttendCompanyWorkshopComplianceView
{
    public function getActivity()
    {
        return new ActivityTrackerActivity(61);
    }
}

class HortonHolding20102011ComplianceProgram extends ComplianceProgram
{
    public function loadGroups()
    {
        ...
        $newHireCallback = function (User $user) {
            return !$user->hiredate || $user->getDateTimeObject('hiredate')->format('U') < strtotime('2010-10-11');
        };
        ...
        $screeningView->setEvaluateCallback($newHireCallback);
        ...
        $safetyWorkshopView = new HortonWorkshop($programStart, $programEnd, 1, true);
        ...
        $ongoingHealthCoachingView = new HortonGraduateFromCoachingSessionComplianceView($programStart, $programEnd);
        ...
        $ongoingHealthCoachingView->setEvaluateCallback($newHireCallback);
...
//
        ...
        $attendWorkshopView = new HortonOptionBasedActivity($programStart, $programEnd, 62, 64);
        ...
        $attendWorkshopView->setDefaultPoints(5);
        $extraGroup->addComplianceView($attendWorkshopView);

        $eLearningLessonsView = new CompleteELearningGroupSets($programStart, $programEnd);
        $eLearningLessonsView->setMaximumNumberOfPoints(35);
        $eLearningLessonsView->addGroup('required', 5);
        $eLearningLessonsView->addGroup('sleep', 10);
        $eLearningLessonsView->addGroup('depression', 10);
        $eLearningLessonsView->setMaximumNumberOfPoints(35);
        $eLearningLessonsView->setReportName('Complete e-Learning Lessons');
        $eLearningLessonsView->setName('elearning');
        $extraGroup->addComplianceView($eLearningLessonsView);
        ...
        $seminarView = new HortonOptionBasedActivity($programStart, $programEnd, 63, 66);
        $seminarView->setName('attend_seminar');
        $seminarView->setReportName('It\'s Cool to Be Healthy SEMINARS Attended');
        $seminarView->setDefaultPoints(10);
        $seminarView->setMaximumNumberOfPoints(20);
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            $printer = new HortonScreeningProgramReportPrinter();
            ...
            $printer = new HortonComplianceProgramReportPrinter();
        }
...
class HortonComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    ...
    <p>Welcome to your summary page for the 2010-2011 Well-Being Rewards Program. To receive the Well-Being Reward,
        eligible employees MUST take action and meet the following requirements by September 30, 2011:</p>
    <ol>
    ...
    <ul>
        <li>Employees on Horton’s medical plan who complete the required core actions and earn 100 or more points from
            key screening results and other available actions by September 30, 2011 will receive a check for 10% of the
            cost of their annual medical premium.
        </li>
...
class HortonScreeningProgramReportPrinter extends ScreeningProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
    ...
    <p>
        <strong>Maximum Possible = 80 points</strong></p>
    <?php
    }
}