...
class LincolnElectric2017SecondCompleteScreeningView extends CompleteScreeningComplianceView
{
    ...
    public function getData(User $user)
    {
        $query = SelectQuery::create()
            ...
            ->andWhere('s.created_at >= ?', array('2018-04-07'));
...
class LincolnElectric2017SecondHomePageComplianceProgram extends ComplianceProgram
{
    ...
    public function loadGroups()
    {
        ...
        $screeningFormView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use($screeningFormView) {
            ...
            if($user->getLocation() == 'RIMROCK CORPORATION' || $user->getLocation() == 'WOLF ROBOTICS LLC') {
                $status->getComplianceView()->addLink(new Link('Download Form', '/resources/9868/2018 Preventive Care form - WOLF and RIMROCK.pdf', false, '_blank'));
                $status->getComplianceView()->addLink(new Link('Submit Form', '/content/chp-document-uploader'));
            } else {
                $status->getComplianceView()->addLink(new Link('Download Form', '/resources/9863/2018 PCP Form 100217.pdf', false, '_blank'));
                $status->getComplianceView()->addLink(new Link('Submit Form', '/content/chp-document-uploader'));
            }
        ...
        $healthLinc2018View->addLink(new Link('Click to View Status', '/compliance_programs?id=1114'));
...
class LincolnElectric2017SecondComplianceProgram extends ComplianceProgram
{
    public function loadGroups()
    {
        ...
        $setEndDate = function ($format, User $user) use ($currentProgram) {
            if($user->getLocation() == 'RIMROCK CORPORATION' || $user->getLocation() == 'WOLF ROBOTICS LLC') {
                $programEnd = '2018-03-31';
            } else {
                $programEnd = '2017-11-17';
            }

            return date($format, strtotime($programEnd));
        };
        ...
        $screeningView = new LincolnElectric2017SecondCompleteScreeningView($programStart, $programEnd);
        ...
        $screeningView->setAttribute('deadline', '11/17/2017');
        $screeningView->emptyLinks();
        $screeningView->setEndDate($setEndDate);
        $screeningView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($currentProgram) {
            $view = $status->getComplianceView();
            if($currentProgram->isCustomLocation($user)) {
                $view->setAttribute('deadline', '03/31/2018');
                $status->getComplianceView()->addLink(new Link('See Results', '/content/989'));
                $status->getComplianceView()->addLink(new Link('<br />Submit Form', '/content/chp-document-uploader'));
                $status->getComplianceView()->addLink(new Link('<br />Download Form', '/resources/9868/2018 Preventive Care form - WOLF and RIMROCK.pdf'));
            } else {
                $status->getComplianceView()->addLink(new Link('See Results', '/content/989'));
                $status->getComplianceView()->addLink(new Link('<br />Submit Form', '/content/chp-document-uploader'));
                $status->getComplianceView()->addLink(new Link('<br />Download Form', '/resources/9863/2018 PCP Form 100217.pdf'));
            }
        ...
        $nicotineFormView->setAttribute('deadline', '11/17/2017');
        $nicotineFormView->emptyLinks();
        $nicotineFormView->addLink(new FakeLink('Admin will enter', '#'));
        $nicotineFormView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($currentProgram) {
            $view = $status->getComplianceView();
            if($currentProgram->isCustomLocation($user)) {
                $view->setAttribute('deadline', '03/31/2018');
            }
        ...
        $bloodPressureView->setEndDate($setEndDate);
        ...
        $ldlCholesterolView->setEndDate($setEndDate);
        ...
        $hdlCholesterolView->setEndDate($setEndDate);
        ...
        $trigView->setEndDate($setEndDate);
        ...
        $glucoseView->setEndDate($setEndDate);
    ...
    public function getAdminProgramReportPrinter()
    {
        ...
        $printer->addMultipleStatusFieldsCallback(function(ComplianceProgramStatus $status) {
            ...
            return array(
                ...
                'Program - 2017 Wellness Screening - Compliant' => $completeScreeningStatus->isCompliant() ? 'Yes' : 'No',
                'Program - 2017 Wellness Screening - Comment' => $completeScreeningStatus->getComment(),
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        $printer = new LincolnElectric2017SecondComplianceProgramReportPrinter();

        return $printer;
    }


    protected function isCustomLocation($user)
    {
        return $user->getLocation() == 'RIMROCK CORPORATION' || $user->getLocation() == 'WOLF ROBOTICS LLC';
    }

}


class LincolnElectric2017SecondComplianceProgramReportPrinter extends BasicComplianceProgramReportPrinter
{
    ...
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        if($user->getLocation() == 'RIMROCK CORPORATION' || $user->getLocation() == 'WOLF ROBOTICS LLC') {
            $endDateText = '03/31/2018';
        } else {
            $endDateText = '11/17/2017';
        }
        ...
        <div class="pageHeading">2018 HealthLinc Workplace Wellness Program</div>
