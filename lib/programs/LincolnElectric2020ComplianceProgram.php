...
class LincolnElectric2020CompleteScreeningView extends CompleteScreeningComplianceView
{
    ...
    public function getData(User $user)
    {
        ...
        ->andWhere('(s.created_at <= ? || s.created_at >= ?)', array('2019-12-31', '2020-11-01'));
...
class LincolnElectric2020HomePageComplianceProgram extends ComplianceProgram
{
    ...
    public function loadGroups()
    {
        ...
        $screeningFormView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use($screeningFormView) {
            ...
            if($activeProgramRecord->id == 1427) {
            ...
            $status->getComplianceView()->addLink(new Link('Download Form', '/resources/10544/2020_PCP_Form_031120.pdf', false, '_blank'));
            $status->getComplianceView()->addLink(new Link('Submit Form', '/content/chp-document-uploader'));

        });
        $group->addComplianceView($screeningFormView);


        $healthLinc2019View = new PlaceHolderComplianceView(ComplianceStatus::NOT_COMPLIANT);
        $healthLinc2019View->setAttribute('always_show_links_when_current', true);
        $healthLinc2019View->setName('healthLinc_2019');
        $healthLinc2019View->setReportName('Track HealthLinc Status 2019 Program');
        $healthLinc2019View->addLink(new Link('Click to View Status', '/compliance_programs?id=1340'));
        $healthLinc2019View->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            $activeProgramRecord = ComplianceProgramRecordTable::getInstance()->find(1340);
        ...
        $group->addComplianceView($healthLinc2019View);
        ...
        $healthLinc2020View->setName('healthLinc_2020');
        $healthLinc2020View->setReportName('Track HealthLinc Status 2020 Program');
        $healthLinc2020View->addLink(new Link('Click to View Status', '/compliance_programs?id=1427'));
        $healthLinc2020View->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            $activeProgramRecord = ComplianceProgramRecordTable::getInstance()->find(1427);
        ...
        $healthLinc2021View = new PlaceHolderComplianceView(ComplianceStatus::NOT_COMPLIANT);
        $healthLinc2021View->setAttribute('always_show_links_when_current', true);
        $healthLinc2021View->setName('healthLinc_2021');
        $healthLinc2021View->setReportName('Track HealthLinc Status 2021 Program');
        $healthLinc2021View->addLink(new Link('Click to View Status', '/compliance_programs?id=1498'));
        $healthLinc2021View->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            $activeProgramRecord = ComplianceProgramRecordTable::getInstance()->find(1498);
        ...
        $group->addComplianceView($healthLinc2021View);
...
class LincolnElectric2020ComplianceProgram extends ComplianceProgram
{
    public function loadGroups()
    {
        ...
        $screeningView = new LincolnElectric2020CompleteScreeningView($programStart, $programEnd);
        ...
        $screeningView->setAttribute('deadline', '10/26/2020');
        ...
        $screeningView->addLink(new Link('<br />Download Form', '/resources/10544/2020_PCP_Form_031120.pdf'));
        ...
        $nicotineFormView->setAttribute('deadline', '10/26/2020');
    ...
    public function getAdminProgramReportPrinter()
    {
        ...
        $printer->addMultipleStatusFieldsCallback(function(ComplianceProgramStatus $status) {
            ...
            return array(
                ...
                'Program - 2020 Wellness Screening - Compliant' => $completeScreeningStatus->isCompliant() ? 'Yes' : 'No',
                'Program - 2020 Wellness Screening - Comment' => $completeScreeningStatus->getComment(),
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        $printer = new LincolnElectric2020ComplianceProgramReportPrinter();
...
class LincolnElectric2020ComplianceProgramReportPrinter extends BasicComplianceProgramReportPrinter
{
    ...
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        <div class="pageHeading">2021 HealthLinc Workplace Wellness Program</div>
    ...
    <?php endif ?>
        ...
        <table class="phipTable">
            ...
            <tr style="height: 20px; border-left: hidden; border-right: hidden;">
                <td colspan="5"></td>
            </tr>

            <tr class="headerRow headerRow-wellness_programs" style="font-size: 12pt; font-weight: bold;">
                <td class="center" colspan="5">Education Purposes Only - Not Tied To Incentive</td>
            </tr>
