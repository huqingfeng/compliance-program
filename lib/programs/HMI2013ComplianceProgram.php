...
class HMI2013ComplianceProgramReportPrinter extends BasicComplianceProgramReportPrinter
{
    ...
    <p>
        Welcome to your summary page for the 2013 Wellness Rewards benefit at HMI.
        This year, we are introducing a new wellness rewards incentive program.
    </p>

    <p>In 2013, the wellness rewards will be:</p>
    <ul>
        <li>Quarterly raffles</li>
        <li>An annual raffle with a grand prize gift to be announced later in the year</li>
    </ul>
    ...
    <p style="text-align:center;"><a href="/content/1094">Click here for more details about the 2013 Wellness Rewards
...
class HMI2013ComplianceProgram extends ComplianceProgram
{
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        $printer = new HMI2013ComplianceProgramReportPrinter();
    ...
    public function loadEvaluators()
    {
        ...
        $q2Deadline = new ProgramStatusEvaluatorComplianceView($this->cloneForEvaluation($this->getStartDate(), '2013-06-30'), array(), 100);
        $q2Deadline->setReportName('Q2 Status - By <strong>06/30/2013</strong>');
        $q2Deadline->setName('q2');
        $totalsGroup->addComplianceView($q2Deadline);

        $q3Deadline = new ProgramStatusEvaluatorComplianceView($this->cloneForEvaluation($this->getStartDate(), '2013-09-30'), array(), 150);
        $q3Deadline->setReportName('Q3 Status - By <strong>09/30/2013</strong>');
        $q3Deadline->setName('q3');
        $totalsGroup->addComplianceView($q3Deadline);

        $q4Deadline = new ProgramStatusEvaluatorComplianceView($this->cloneForEvaluation($this->getStartDate(), '2013-12-31'), array(), 200);
        $q4Deadline->setReportName('Q4 Status - By <strong>12/31/2013</strong>');
        $q4Deadline->setName('q4');
        $totalsGroup->addComplianceView($q4Deadline);

        $goldView = new ProgramStatusEvaluatorComplianceView($this->cloneForEvaluation($this->getStartDate(), '2013-12-31'), array(), 300);
        $goldView->setReportName('Annual Grand Raffle Status - By <strong>12/31/2013</strong>');
