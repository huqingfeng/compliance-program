...
class RSUI2017ComplianceProgramReportPrinter extends CHPStatusBasedComplianceProgramReportPrinter
{
...
class RSUI2017ComplianceProgram extends ComplianceProgram
{
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        $printer = new RSUI2017ComplianceProgramReportPrinter();
    ...
    {
        return new RSUI2017ComplianceProgramAdminReportPrinter();
    }
    ...
    public function loadGroups()
    {
        ...
        $physicalAlternativeView->setAllowPointsOverride(true);
...
class RSUI2017ComplianceProgramAdminReportPrinter implements ComplianceProgramAdminReportPrinter
{
    public function printAdminReport(ComplianceProgramReport $report, $output)
    {
        ...
        foreach($report as $status) {
            ...
            $health_score_override = $user->getNewestDataRecord('health_score_override');
            $health_score_override = $health_score_override->getDataFieldValue('health_score');
            $health_score_compliant_override = ($health_score_override >= 250) ? true : false;
            ...
            $fields = array(
                ...
                'Health Score'           => ($health_score_override)? $health_score_override : $status->getComplianceViewGroupStatus('Health Score')->getPoints(),
                'Overall - Compliant'    => ($status->isCompliant() || $health_score_compliant_override) ? 'Yes' : 'No'
            );

            if($user->relationship_type == \Relationship::EMPLOYEE && !($user->hasAttribute(8192))) {
                if ($user->id !== "2601628"){
                    $employees[$user->id] = $fields;
                }

                if ($user->relationship_user_id) {
