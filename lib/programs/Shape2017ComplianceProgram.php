...
class ShapePrinter extends CHPShapeComplianceProgramReportPrinter
{
    public function printHeader(ComplianceProgramStatus $status)
    {
        ...
        <p style="font-weight:bold">
            The 2017-2018 SFW Annual Requirements must be met between March 1, 2017 and May 1, 2017. Please view the list
            of requirements below; each requirement shows related details and has a light indicator showing if it has
            been completed, is incomplete, or is not required of you this year.
        </p>
...
class Shape2017BMINicotineCardComplianceProgramView extends PlaceHolderComplianceView
{
    public function getStatus(User $user)
    {
        ...
        foreach($records as $record) {
            if($record->exists()
                && $record->nicotine_use != ''
                && date('Y-m-d', strtotime($record->getCreationDate())) >= '2017-01-01'
                && date('Y-m-d', strtotime($record->getCreationDate())) <= '2017-06-30') {
                $nicotineUse = true;
            }
        }
...
class Shape2017FamilyWellnessComplianceProgramView extends ComplianceView
{
    ...
    {
        $appintmentTimes = AppointmentTime::getScheduledAppointments($user, '2017-05-01');
    ...
    const Shape2017BMINICOTINESTARTDATE = '2017-03-01';
    const Shape2017BMINICOTINEENDDATE = '2017-05-01';
}

class Shape2017WeightManagementNicotineProgramView extends PlaceHolderComplianceView
{
    const SHAPE_2016_COACHING_RECORD_ID = 774;
    public function getStatus(User $user)
    {
        if($user->hasAttribute(Attribute::COACHING_END_USER)) {
            $program = ComplianceProgramRecordTable::getInstance()->find(self::SHAPE_2016_COACHING_RECORD_ID);
...
class Shape2017ComplianceProgramAdminReportPrinter extends BasicComplianceProgramAdminReportPrinter
{
...
class Shape2017ComplianceProgram extends ComplianceProgram
{
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        return new Shape2016WMS2Printer();
    }

    public function getAdminProgramReportPrinter()
    {
        $printer = new Shape2017ComplianceProgramAdminReportPrinter();
...
class Shape2016WMS2Printer implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        <div class="row">
            <div class="col-md-12">
                <div id="header-text">
                    <p style="font-weight:bold">
                        The 2017-2018 SFW Annual Requirements must be <span style="color:red;">completed</span> between March 1, 2017 and May 1, 2017.
