...
class Wheels2015ComplianceProgram extends ComplianceProgram
{
    const POINTS = 81;
    ...
    public function getAdminProgramReportPrinter()
    {
        ...
        $printer->addMultipleStatusFieldsCallback(function(ComplianceProgramStatus $status) {
            $oldPoints = $status->getComplianceViewGroupStatus('2014_points')->getPoints();
            $newPoints = $status->getComplianceViewGroupStatus('points')->getPoints();

            return array(
                '2014_points'                    => $oldPoints,
                '2015_points'                    => $newPoints,
                '2015_2014_change'               => bcsub($newPoints, $oldPoints, 1),
                Wheels2015ComplianceProgram::POINTS.'+ in 2014, or 3+ improvement' => $status->getComplianceProgram()->isCompliantFor1C($oldPoints, $newPoints)
            );
        });
    ...
    {
        return $newPoints >= Wheels2015ComplianceProgram::POINTS || ($oldPoints > 0 && $newPoints - $oldPoints >= 3);
    }
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            $printer = new Wheels2015ScreeningTestPrinter();
            $printer->setShowLight(true);
            $printer->setShowPoints(false);

            return $printer;
        } else {
            $printer = new Wheels2015ComplianceProgramReportPrinter();
        }
    ...
    {
        $oldProgramRecord = ComplianceProgramRecordTable::getInstance()->find(380);

        $screeningTestAlias = 'wheels_2015';
        $lastYearsTestAlias = 'wheels_2014';
        ...
        $oldPointsGroup = new ComplianceViewGroup('2014_points', '2014 Numbers');

        $screeningModel = new ComplianceScreeningModel();

        $startDate = $this->getStartDate();
        $endDate = $this->getEndDate();
        $coreEndDate = '2016-06-30';
        ...
        $oldScreeningTestViews = $this->getScreeningTestViews($screeningModel, $lastYearsTestAlias, '2014-06-01', '2015-07-31', new \DateTime('2014-11-05'), '', $oldProgramRecord, null);

        $core = new ComplianceViewGroup('core', 'Core actions required by 10/31/2015');

        $scrView = new CompleteScreeningComplianceView($startDate, $coreEndDate);
        $scrView->setReportName('Complete Wellness Screening');
        $scrView->setAttribute('deadline', '10/31/2015');
        ...
        $hpaView->setAttribute('deadline', '10/31/2015');
...
class Wheels2015ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        $oldBiometricPoints = $status->getComplianceViewGroupStatus('2014_points')->getPoints();
        ?>
        <style type="text/css">
        ...
        <p>Please allow at least 5-10 days after your 2015 wellness screening for your results and points to show.</p>
        ...
        <table class="phipTable" border="1">
            ...
            <tr>
                <td><a href="/content/1094#1bHPA">C. Get <?php echo Wheels2015ComplianceProgram::POINTS ?> biometric points from your 2015 Wellness Screening; OR; get 3 or more biometric points over last year's total.</a>
                </td>
                <td class="center">
                    <?php echo $completeScreeningStatus->getComplianceView()->getAttribute('deadline') ?>
                </td>
            ...
            <tr>
                <td colspan="2" class="left">
                    ...
                    <div style="text-align:right;">A. Total Biometric Points for 2015 = </div>
                    <div style="text-align:right;">B. Total Points from 2014 = </div>
                    <div style="text-align:right;">-------------------------------------------- </div>
                    <div style="text-align:right;">C. Point Increase or (Decrease) in 2015 =</div>
                </td>
                <td class="center">
...
class Wheels2015ScreeningTestPrinter extends ScreeningProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ?>
