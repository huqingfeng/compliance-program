<?php
class WHC2012ScreeningPrinter extends ScreeningProgramReportPrinter
{
...
class WHC2012CoachingView extends GraduateFromCoachingSessionComplianceView
{
...
class CulverWorkshop2012 extends AttendCompanyWorkshopComplianceView
{
...
class WHC2012LifetimeActivityView extends CompleteActivityComplianceView
{
...
class WHC2012ComplianceProgram extends ComplianceProgram
{
    public function loadEvaluators()
    {
        $raffleGroup = new ComplianceViewGroup('raffle', 'Raffle Eligibility');

        $aprilTotal = new ProgramStatusEvaluatorComplianceView(
            $this->cloneForEvaluation($this->getStartDate(), '2012-04-15'),
            array('core'),
            200,
            false
        );
        $aprilTotal->setName('april_total');
        //$aprilTotal->setReportName('New April Early Bird Raffle! 4/15/12 Deadline & Status * =');
        $raffleGroup->addComplianceView($aprilTotal, true);
    ...
    public function loadGroups()
    {
        ...
        $coreGroupEnd = '2012-08-15';

        $programEnd = strtotime('2012-11-15');

        $notSpouse = function (User $user) {
            return $user->relationship_type != Relationship::SPOUSE;
        };

        $coreGroup = new ComplianceViewGroup('core', 'All Core Actions Required by '.date('F d, Y', strtotime($coreGroupEnd)));
        ...
        current($requiredLessons->getLinks())->setLinkText('Do Required Lessons');
        $coreGroup->addComplianceView($requiredLessons);

        $this->addComplianceViewGroup($coreGroup);

        // Build the extra group
        $extraGroup = new ComplianceViewGroup('points', 'And, earn 200 or more points from A-D below by '.date('F d, Y', $programEnd));
...
//
        ...
        $waistView->setComplianceStatusPointMapper($screeningTestMapper);
        $waistView->setReportName('Waist Size');
        $waistView->setName('waist');
        $extraGroup->addComplianceView($waistView);
        ...
        $preventiveExamsView = new WHC2012LifetimeActivityView(
        ...
        $fluVaccineView = new WHC2012LifetimeActivityView(
            $programStart, $programEnd, 60, 63, 5
        );
        $fluVaccineView->setMaximumNumberOfPoints(20);
        $fluVaccineView->setReportName('Get an annual flu shot immunization ');
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            $printer = new WHC2012ScreeningPrinter();
            ...
            $printer = new WHC2012ComplianceProgramReportPrinter();
        }
...
class WHC2012ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        $waistStatus = $pointGroupStatus->getComplianceViewStatus('waist');
        ...
        $aprilStatus = $raffleGroupStatus->getComplianceViewStatus('april_total');
        ?>
    <style type="text/css">
    ...
    <p></p>
    <p>Welcome to your summary page for the 2011-2012 Wellness Health Check.</p>

    <p>To receive the incentives, eligible employees and spouses MUST take certain actions and meet the criteria
        specified below:</p>
    <ol>
        <li>1. Complete ALL core actions listed in table below by August 15, 2012 _______ TBD : AND</li>
        <li>2. Earn at least 200 points through the options in the table below by November 15, 2012 ________ (TBD)</li>
    </ol>

    <p>By meeting the above requirements, eligible employees/families will receive the following rewards:</p>
    <ol style="list-style-type:upper-alpha;">
        <li>$ _____ (TBD) as a….. premium contribution credit, flex benefit contribution, cash, etc. (TBD)</li>
    </ol>
    <p>Important: The incentive requirements apply to each eligible employee and spouse with medical benefits through
        _____ (employer name). </p>
    <p>In addition, by taking these actions and getting more points:</p>
    <ul>
    ...
    <tr>
        <td><a href="/content/1094#1bHPA">B. <?php echo $completeHRAStatus->getComplianceView()
            ->getReportName() ?></a></td>
        <td class="center">
    ...
    <tr>
        <td><a href="/content/1094#1bHPA">C. <?php echo $elearningStatus->getComplianceView()
            ->getReportName() ?></a></td>
        <td class="center">
    ...
    <tr>
        ...
        <td class="center"><?php echo $totalCholesterolStatus->getComplianceView()
    ...
    <tr>
        <td>
            <ul>
                <li><?php echo $waistStatus->getComplianceView()->getReportName() ?></li>
            </ul>
        </td>
    ...
    <tr>
        <td><a href="/content/1094#2clmm">B. <?php echo $fluVaccineStatus->getComplianceView()
            ->getReportName() ?></a></td>
        ...
        <td class="links">
            <a href="/content/12048?action=showActivity&activityidentifier=60">Sign-Up @ Work Form if Elsewhere</a>
        </td>
    </tr>
    <tr>
        <td><a href="/content/1094#2dRPA">C. <?php echo $physicalActivityStatus->getComplianceView()
            ->getReportName() ?></a></td>
    ...
    <tr>
        <td><a href="#">D. Complete lessons to improve results in 2A</a></td>
    ...
    <tr>
        <td><a href="/content/1094#2geLearn">E. <?php echo $extraELearningStatus->getComplianceView()
            ->getReportName() ?></a></td>
    ...
    <tr>
        ...
        <td style="text-align:center">380 points possible!</td>
    </tr>
    <tr class="headerRow">
    ...
    <tr>
        <td style="text-align: right;">My Core Actions & Points as of: <?php echo date('m/d/Y'); ?> =</td>
