...
class Henniges2023CompleteScreeningComplianceView extends CompleteScreeningComplianceView
{
...
class Henniges2023CompleteScreeningAlternativeComplianceView extends CompleteScreeningComplianceView
{
    public function getData(User $user)
    {
        $data = ScreeningTable::getInstance()->findOneCompletedForUserBetweenDates($user,
            ...
            array(
                ...
                'require_complete' => false,
                'filter'           => $this->getFilter(),
                'required_fields'  => array('cholesterol', 'ha1c')
            )
        );
...
class Henniges2023ComplianceProgram extends ComplianceProgram
{
    public function getAdminProgramReportPrinter()
    {
        ...
        $printer->addCallbackField('paygroup', function (User $user) {
            return $user->getInsurancePlanType();
        });
    ...
    {
        return new Henniges2023WMS2Printer();
    }
    ...
    public function loadGroups()
    {
        ...
        $HraProgramStart = '2023-01-01';

        $preventionEventGroup = new ComplianceViewGroup('required', 'Prevention Events');


        $hraView = new CompleteHRAComplianceView($HraProgramStart, $programEnd);
        $hraView->setStatusSummary(ComplianceStatus::COMPLIANT, 'Complete');
        $hraView->setReportName('Health Risk Appraisal (HRA)');
        $hraView->setName('hra');
        $hraView->emptyLinks();
        $hraView->setAttribute('deadline', date("m/d/Y", $programEnd));
        $hraView->addLink(new Link('Take HRA', sfConfig::get('app_wms2') ? '/compliance/henniges-automotive-2023/hra/content/my-health' : '/content/989'));
        $preventionEventGroup->addComplianceView($hraView);

        $screeningView = new Henniges2023CompleteScreeningComplianceView($programStart, $programEnd);
        ...
        $screeningView->setAttribute('deadline',  date("m/d/Y", $programEnd));
        $screeningView->setPostEvaluateCallback(function($status, $user) use($programStart, $programEnd) {
            $alternativeView = new Henniges2023CompleteScreeningAlternativeComplianceView($programStart, $programEnd);
            $alternativeStatus = $alternativeView->getStatus($user);

            if($status->getStatus() != ComplianceStatus::COMPLIANT && $alternativeStatus->getStatus() == ComplianceStatus::COMPLIANT) {
        ...
        $cotinineView->setAttribute('deadline',  date("m/d/Y", $programEnd));
        $cotinineView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            ...
            if ($client_id == 3751 || $relationship != Relationship::EMPLOYEE) {
                $status->setStatus(ComplianceStatus::NA_COMPLIANT);
                $status->getComplianceView()->setAttribute('deadline', 'Not Required');
            }
    ...
    public function evaluateAndStoreOverallStatus(ComplianceProgramStatus $status)
    {
        ...
        foreach($requiredGroupStatus->getComplianceViewStatuses() as $viewStatus) {
            ...
            if($status->getUser()->getClientId() == 3751 && $viewName == 'tobacco_status') continue;
        ...
        if($status->getComplianceViewStatus('screening')->getStatus() == ComplianceStatus::COMPLIANT) {
...
class Henniges2023WMS2Printer implements ComplianceProgramReportPrinter
{
    ...
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        $groupTable = function(ComplianceViewGroupStatus $group) use ($classFor, $classForStatus, $textForStatus, $textForSpouseStatus, $that, $user) {
            ob_start();
        ...
        $tableRow = function($name, ComplianceViewGroupStatus $group) use ($classFor, $classForStatus, $textForStatus, $groupTable, $user) {
            ob_start();
            ...
            foreach($group->getComplianceViewStatuses() as $viewStatus) {
                ...
                if(($user->getClientId() == 3751 || $relationship != Relationship::EMPLOYEE) && $viewName == 'tobacco_status') continue;
        ...
        <div class="row">
            <div class="col-md-12">
                <div id="more-info">
                    <div class="row">
                        <div class="col-md-12">
                            <a href="/content/henniges-previous-program-years">Previous Program Years</a><br />
                        </div>
                        <div class="col-md-12">
                            <?php if($user->getClientId() == 3748) : ?>
                                ...
                                <p>Click on <a href="/compliance/henniges-automotive-2021/program-overview/content/henniges_program_overview_2022">Program Overview</a> link to view your incentive program details and requirements.</p>
                            ...
                            <?php elseif($user->getClientId() == 3751) : ?>
                                ...
                                <p>Click on <a href="/compliance/henniges-automotive-2023/program-overview/content/henniges_program_overview_2023">Program Overview</a> link to view your incentive program details and requirements.</p>
