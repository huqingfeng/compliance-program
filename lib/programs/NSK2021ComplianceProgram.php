<?php
  ...
  class NSK2021CompleteScreeningComplianceView extends CompleteScreeningComplianceView {
  ...
  class NSK2021ComplianceProgram extends ComplianceProgram {
    public function getProgramReportPrinter($preferredPrinter = null) {
      return new NSK2021WMS2Printer();
    }
    ...
    public function getAdminProgramReportPrinter() {
      ...
      $printer->addMultipleStatusFieldsCallback(function(ComplianceProgramStatus $status) {
          ...
          $data['Phone Consultation Showed'] = $this->getPhoneConsultShowed($user, $status);
    ...
    protected function getPhoneConsultShowed($user, $status) {
      $datesToAccept = array(date('Y-m-d', strtotime($status->getAttribute('screening_date'))), date('Y-m-d', strtotime(date('Y-m-d', strtotime($status->getAttribute('screening_date'))) . ' + 40 days')));
      ...
      $row = SelectQuery::create()
        ...
        ->andWhere('a.date BETWEEN ? AND ?', $datesToAccept)
    ...
    public function loadGroups() {
      ...
      $screeningBiometric = new NSK2021CompleteScreeningComplianceView($programStart, $programEnd);
      ...
      $glucoseView->setReportName('Better of Glucose or HA1C');
      ...
      $glucoseView->setStatusSummary(ComplianceStatus::COMPLIANT, 'Glucose: 65-99, HA1C: 4.8-5.9');
      $glucoseView->setStatusSummary(ComplianceStatus::PARTIALLY_COMPLIANT, 'Glucose: 50-64 or 100-125, HA1C: < 4.8 or > 5.9');
      $glucoseView->setStatusSummary(ComplianceStatus::NOT_COMPLIANT, 'Glucose: ≤ 49 or ≥ 126, HA1C: < 4.32 or > 8');
      $glucoseView->overrideTestRowData(50, 65, 99, 125);
      ...
      if($ha1cStatus->getStatus() == ComplianceStatus::COMPLIANT && $status->getPoints() <= 2) {
          $status->setPoints(2);
          $status->setComment("HA1C: ".$ha1cResult);
          $status->setStatus(ComplianceStatus::COMPLIANT);
      } elseif($ha1cStatus->getStatus() == ComplianceStatus::PARTIALLY_COMPLIANT && $status->getPoints() <= 1) {
          $status->setPoints(1);
          $status->setComment("HA1C: ".$ha1cResult);
          $status->setStatus(ComplianceStatus::PARTIALLY_COMPLIANT);
      } else {
          $status->setComment("Glucose: ".$glucoseResult);
      }
      });
    ...
    protected function configureViewForElearningAlternative(ComplianceView $view, $alias, $allAliases) {
        ...
        $view->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use(&$callCache, $alias, $allAliases, $userIdsToIgnore) {
            ...
            if($view->getStartDate('Y-m-d') != '2021-01-01') {
                return;
            }
    ...
    protected function evaluateAndStoreOverallStatus(ComplianceProgramStatus $status) {
        ...
        if($this->getPhoneConsultShowed($user, $status)) {
        ...
        if($screeningStatus->getComment() == date('m/d/Y', strtotime($screeningStatus->getComment()))) {
            $startDate = '2021-01-01';
            $endDate = date('m/d/Y', strtotime('+40 days', strtotime($screeningStatus->getComment())));
            ...
            $appt = SelectQuery::create()
                ...
                ->where('a.date BETWEEN ? AND ?', array($startDate, $endDate))
        ...
        if ($this->isNewHire($status->getUser())) {
            ...
            if($status->isCompliant()) {
                $status->setComment('$0');
            } elseif(($thisPreventionCompliant) && ($thisTobacco->isCompliant() || $biometricPoints >= 10 || $this->getPhoneConsultShowed($user, $status))) {
    ...
    const RECORD_ID = 1593;
  }


  class NSK2021WMS2Printer implements ComplianceProgramReportPrinter {
      ...
      public function printReport(ComplianceProgramStatus $status) {
          ...
          <div class="row">
              <div class="col-md-12">
                  <a href="/compliance/nsk-2021/overview">Full Program Details</a><br/>
          ...
          <div class="row">
            <div class="ticker <?= !$tickerCountShow ? 'hide' : '' ?>">
              <style>
                .ticker {
                  width: 100%;
                  display: flex;
                ...
                .ticker .right {
                  flex-grow: 1;
                  border-top: 2px solid #CAD4D9;
                  border-right: 2px solid #CAD4D9;
              ...
              <div class="left">
                <p class="days"><?= $tickerCountDown ?></p>
                <p class="remaining">DAYS REMAINING</p>
              </div>
              <div class="right">
                <p><?= $tickerCountDown ?> days to complete consultation <b>IF</b> you have less than 10 points on your Report Card</p>
              </div>
            </div>
          </div><br />
          ...
          <div class="row">
              <div class="col-md-12">
                  <div class="alert alert-warning" style="color: #333; background-color: rgb(253 184 59 / 20%); border: 4px solid #fdb83b;">
                      HRA’S MUST BE COMPLETED ON-LINE PRIOR TO SCHEDULING YOUR SCREENING.<br>
                      SCREENING RESULTS SHOULD BE AVAILABLE IN YOUR REPORT CARD WITHIN 5-7 CALENDAR DAYS OF YOUR SCREENING DATE.<br>
                      AFTER YOUR SCREENING, REVIEW YOUR REPORT CARD.<br>
                      IF YOU HAVE NOT ACCUMULATED 10 OR MORE POINTS IN YOUR REPORT CARD YOU MUST SCHEDULE YOUR PHONE CONSULTATION WITH CIRCLE WELLNESS USING THE <strong>SCHEDULE APPOINTMENTS</strong> BUTTON ON THE HOME PAGE OR BY CONTACTING CIRCLE WELLNESS AT <strong>866-682-3020 X204</strong> FOR ASSISTANCE.<br>
                      PHONE CONSULTATIONS MUST BE SCHEDULED AS SOON AS YOUR FULL SCREENING RESULTS ARE POPULATED IN YOUR REPORT CARD AND PHONE CONSULTATIONS *MUST* BE COMPLETED WITHIN 40 CALENDAR DAYS OF YOUR SCREENING DATE.<br>
                      PLEASE DO NOT DELAY IN SCHEDULING YOUR PHONE CONSULTATION WITH CIRCLE WELLNESS.<br>
                      SAME DATE, OR NEXT DAY, PHONE CONSULTATIONS ARE VERY UNLIKELY, ESPECIALLY DURING PEAK WELLNESS SCREENING MONTHS.<br>
                      TRY TO SCHEDULE YOUR PHONE CONSULTATION AS SOON AS POSSIBLE, ALLOWING YOURSELF 3-4 WEEKS BEFORE THE 40 CALENDAR DAY DEADLINE.<br>
                      NOTE:  PARTICIPANTS HAVE THE ABILITY TO SCHEDULE PHONE CONSULTATIONS 24/7 USING THE <strong>SCHEDULE APPOINTMENTS</strong> BUTTON ON THE HOME PAGE; HOWEVER, CIRCLE CUSTOMER SERVICE REPRESENTATIVES ARE ONLY AVAILABLE MONDAY THROUGH FRIDAY FROM 8:30 AM TO 6:00 PM EST.<br>
                  </div>
              </div>
          </div>
          <div class="row">
              <div class="col-md-12">
                  ...
                  <p>
                      ...
                      <a href="/compliance/nsk-2021/my-report-card/pdf/clients/nsk/NSK_2021_Pregnancy_Exception_Form.pdf">pregnancy exception form</a>
