...
class GetUpAndGoWMS3ExerciseToSteps extends CompleteArbitraryActivityComplianceView
{
...
class GetUpAndGoWMS3FitbitComplianceView extends DateBasedComplianceView
{
    public function __construct($startDate, $endDate, $domain)
    {
    ...
    public function getStatus(User $user)
    {
        ...
        $url = $domain."wms3/public/report?bypass=true&method=download&program_id=".$program_id."&program_name=".$guagDataType."&type=get_up_and_go&return=true";
        ...
        if (empty($result)) {
            ...
            foreach($campaign_data as $data) {
                if ($data->id == $user->id) {
                    ...
                    $average_steps = $data->daily_steps_average;
                    $manual_steps = $data->device_5_total_steps;
                }
            }
        }
        ...
        $status->setAttribute('manual_steps', $manual_steps);
        $status->setAttribute('tracker_steps', $tracker_steps);
        $status->setAttribute('average_steps', $average_steps);
...
class GetUpAndGoWMS3WMS2AdminPrinter extends BasicComplianceProgramAdminReportPrinter
{
...
class GetUpAndGoWMS3ComplianceProgram extends ComplianceProgram
{
    public function getAdminProgramReportPrinter()
    {
        $record = $this->getComplianceProgramRecord();

        return new GetUpAndGoWMS3AdminPrinter($record);
    }

    public function getTeamDashboardPrinter()
    {
        return new GetUpAndGoWMS3TeamDashboardPrinter();
    }

    public function getBuddyDashboardPrinter()
    {
        return new GetUpAndGoWMS3BuddyDashboardPrinter();
    }

    public function getProgramReportPrinter($preferredPrinter = null)
    {
        return new GetUpAndGoWMS3ProgramReportPrinter();
    }
    ...
    public function loadGroups()
    {
        ...
        $fitbitView = new GetUpAndGoWMS3FitbitComplianceView($startDate, $endDate, $domain);
        ...
        <span style="font-size: 1.'. (sfConfig::get('app_wms2', false) ? '5' : '0715') . 'rem; display: inline-block; margin: 1rem 0;">To sync to Apple Health or Google Fit, <a href="/resources/10560/2020_CircleWellness_App_Tracker_Syncing.pdf" target="_blank">follow these directions</a> on our Circle Well App.</span>
    ...
    public function getActionTemplateCustomizations()
    {
        ...
        <?php if (!sfConfig::get('app_wms2', false)): ?>
          <?php if (!loadRegister()): ?>
              <button id="optinButton">I would like to participate in the Get Up and Go Campaign.</button>
          <?php endif; ?>
          <div id="wms1-content" style="max-width:900px;margin:0 auto;">
        <?php endif; ?>
        ...
        <style>
            ...
            .selection_confirmation {
                ...
                font-size: 1.<?= sfConfig::get('app_wms2', false) ? '5' : '0715'?>rem;
            }
        ...
        <style type="text/css">
            ...
            #wms1<?= sfConfig::get('app_wms2', false) ? '' : '-content' ?> * {
                display: none;
            }
        ...
        <script type="text/javascript">
            $(function() {
                ...
                $('#optinButton').click(function(){
                    $.ajax({
                        method: "GET",
                        url: "<?= sfConfig::get('app_wms2', false) ? '/wms1' : '' ?>/content/guag-opt-in",
                        dataType: "json",
...
class GetUpAndGoWMS3ProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        <style type="text/css">
            ...
            .table td > a {
              ...
              padding: <?= sfConfig::get('app_wms2', false) ? '1.2' : '0.7'?>5rem <?= sfConfig::get('app_wms2', false) ? '2' : '1'?>.25rem;
              border: 1px solid #4696ec;
              text-transform: uppercase;
              margin-right: <?= sfConfig::get('app_wms2', false) ? '3' : '2'?>rem;
              margin-bottom: <?= sfConfig::get('app_wms2', false) ? '3' : '2'?>rem;
              position: relative;
            }
        ...
        <?php if (!loadRegister() && sfConfig::get('app_wms2', false)): ?>
            <button id="optinButton">I would like to participate in the Get Up and Go Campaign.</button>
        <?php endif; ?>
            ...
            <tbody>
                ...
                <tr>
                    <td>
                        ...
                        <?php else: ?>
                          <a href="/compliance_programs?id=1156&forceRefresh=true&wms1Id=<?= $user_id ?>&device=<?=$device_id?>">Log Steps Manually</a>
                        <?php endif; ?>
                    </td>
                </tr>
            </tbody>
        ...
        <table class="table table-condensed table-striped">
            <tbody>
                ...
                <tr>
                    ...
                    <td style="text-align:center"><?php echo number_format($viewStatus->getAttribute("manual_steps")) ?></td>
                </tr>
                <tr>
                    <th>
                        Fitness Tracker Synced Steps
                    </th>
                    <td style="text-align:center"><?php echo number_format($viewStatus->getAttribute("tracker_steps")) ?></td>
                </tr>
                <tr>
                    <th style="text-align:left">Total Number of Steps</th>
                    <td style="text-align:center"><?php echo number_format($viewStatus->getPoints()) ?></td>
                </tr>
                <tr>
                    <th style="text-align:left">Average Daily Steps</th>
                    <td style="text-align:center"><?php echo number_format($viewStatus->getAttribute("average_steps")) ?></td>
                </tr>
            </tbody>
        </table>
        <?php if (!sfConfig::get('app_wms2', false)): ?>
...
class GetUpAndGoWMS3TeamDashboardPrinter extends GetUpAndGoWMS3BuddyAndTeamDashboardPrinter implements TeamDashboardPrinter
{
...
class GetUpAndGoWMS3BuddyDashboardPrinter extends GetUpAndGoWMS3BuddyAndTeamDashboardPrinter implements BuddyDashboardPrinter
{
...
abstract class GetUpAndGoWMS3BuddyAndTeamDashboardPrinter
{
