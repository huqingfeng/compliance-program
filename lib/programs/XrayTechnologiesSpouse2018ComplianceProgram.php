...
class XrayTechnologiesSpouse2018ComplianceProgram extends ComplianceProgram
{
...
//            $data['Has Biometric Override'] = $status->getAttribute('has_biometric_override') ? 'Yes' : 'No';
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        if (sfConfig::get('app_wms2', true)) {
            return new XrayTechnologiesSpouse2018WMS2Printer();
        } else {
            $printer = new XrayTechnologiesSpouse2018Printer();
    ...
    public function loadGroups()
    {
        ...
        $coachingOverall->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            $notRequired = SelectQuery::create()
                ...
                ->andWhere('compliance_program_record_id = ?', array(XrayTechnologiesSpouse2018ComplianceProgram::XRAYTECHNOLOGIES_SPOUSE_2018_RECORD_ID))
        ...
        $employeeParticipation->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            ...
            if($user->getRelationshipType() != Relationship::SPOUSE || !$employeeUser || (isset($this->options['in_employee']) && $this->options['in_employee']))  {
                $status->setStatus(ComplianceStatus::NA_COMPLIANT);
            } else {
                $program = ComplianceProgramRecordTable::getInstance()->find(XrayTechnologiesEmployee2018ComplianceProgram::XRAYTECHNOLOGIES_EMPLOYEE_2018_RECORD_ID)->getComplianceProgram(array('in_spouse' => true));
...
//            }
    ...
    public function evaluateAndStoreOverallStatus(ComplianceProgramStatus $status)
    {
        ...
        if(!isset($this->options['in_employee']) || !$this->options['in_employee']) {
            $priorStatus = $status->getComplianceProgram()->getPriorYearBiometricsComplianceViewStatuses($status->getUser(), '2016-08-01', '2017-07-30');
    ...
    const XRAYTECHNOLOGIES_SPOUSE_2018_RECORD_ID = 1260;
}

class XrayTechnologiesSpouse2018Printer  extends CHPStatusBasedComplianceProgramReportPrinter
{
...
class XrayTechnologiesSpouse2018WMS2Printer implements ComplianceProgramReportPrinter
{
