...
class VillageOfLibertyville2017WeeklyLogComplianceView extends CompleteActivityComplianceView
{
    static $validWeeks = array(
        1 => '09/11/2017 through 09/17/2017',
        2 => '09/18/2017 through 09/24/2017',
        3 => '09/25/2017 through 10/01/2017',
        4 => '10/02/2017 through 10/08/2017',
        5 => '10/09/2017 through 10/15/2017',
        6 => '10/16/2017 through 10/22/2017',
        7 => '10/23/2017 through 10/29/2017',
        8 => '10/30/2017 through 11/05/2017',
        9 => '11/06/2017 through 11/12/2017',
        10 => '11/13/2017 through 11/19/2017',
        11 => '11/20/2017 through 11/26/2017',
        12 => '11/27/2017 through 12/03/2017',
        13 => '12/04/2017 through 12/10/2017',
        14 => '12/11/2017 through 12/17/2017',
        15 => '12/18/2017 through 12/24/2017',
        16 => '12/25/2017 through 12/31/2017',
        17 => '01/01/2018 through 01/07/2018',
        18 => '01/08/2018 through 01/14/2018',
        19 => '01/15/2018 through 01/21/2018',
        20 => '01/22/2018 through 01/28/2018',
        21 => '01/29/2018 through 02/04/2018',
        22 => '02/05/2018 through 02/11/2018',
        23 => '02/12/2018 through 02/18/2018',
        24 => '02/19/2018 through 02/25/2018',
        25 => '02/26/2018 through 03/04/2018',
        26 => '03/05/2018 through 03/11/2018',
        27 => '03/12/2018 through 03/18/2018',
        28 => '03/19/2018 through 03/25/2018',
        29 => '03/26/2018 through 03/31/2018'
    );
...
class VillageOfLibertyville2017TobaccoFormComplianceView extends ComplianceView
{
...
class VillageOfLibertyville2017ComplianceProgram extends ComplianceProgram
{
    ...
    public function loadGroups()
    {
        ...
        $coreEndDate = '2017-11-17';

        $quarterlyDateRange = $this->getQuerterlyRanges();

        // Build the core group
        $coreGroup = new ComplianceViewGroup('core', 'Areas A-C are required by November 17, 2017 in order to earn the $50 gift card.');
...
//        $physicalActivityView->setMonthlyPointLimit(30);
        ...
        $steps8K->addLink(new Link('Sync Moves App', '/standalone/moves'));
        ...
        $water->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($quarterlyDateRange) {
            foreach($quarterlyDateRange as $quarterName => $dateRange) {
                ...
                $alternative = new VillageOfLibertyville2017WeeklyLogComplianceView($startDate, $endDate, 613, 213, 5);
                $alternative->setUseOverrideCreatedDate(true);
                $alternativeStatus = $alternative->getStatus($user);
        ...
        $vegetable->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($quarterlyDateRange) {
            foreach($quarterlyDateRange as $quarterName => $dateRange) {
                ...
                $alternative = new VillageOfLibertyville2017WeeklyLogComplianceView($startDate, $endDate, 620, 213, 5);
                $alternative->setUseOverrideCreatedDate(true);
                $alternativeStatus = $alternative->getStatus($user);
        ...
        $fruit->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($quarterlyDateRange) {
            foreach($quarterlyDateRange as $quarterName => $dateRange) {
                ...
                $alternative = new VillageOfLibertyville2017WeeklyLogComplianceView($startDate, $endDate, 612, 213, 5);
                $alternative->setUseOverrideCreatedDate(true);
                $alternativeStatus = $alternative->getStatus($user);
        ...
        $nonSmokerView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($quarterlyDateRange) {
            ...
            foreach($quarterlyDateRange as $quarterName => $dateRange) {
                ...
                $alternative = new VillageOfLibertyville2017TobaccoFormComplianceView($startDate, $endDate);
                $alternative->setUseOverrideCreatedDate(true);
                $alternativeStatus = $alternative->getStatus($user);
        ...
        $doc->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($quarterlyDateRange) {
            ...
            foreach($quarterlyDateRange as $quarterName => $dateRange) {
                ...
                if(date('Y-m-d') >= $startDate && date('Y-m-d') <= $endDate) {
                    ...
                    if($alternativeStatus->getStatus() == ComplianceStatus::COMPLIANT && ($quarterName == 'quarter1' || !$quarterOneEarned)) {
    ...
    {
        $ranges = array(
            'quarter1' => array('2017-09-11', '2017-12-31'),
            'quarter2' => array('2018-01-01', '2018-03-31')
        );
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            ...
            $printer = new VillageOfLibertyville2017ComplianceProgramReportPrinter();
...
class VillageOfLibertyville2017ComplianceProgramReportPrinter extends BasicComplianceProgramReportPrinter
{
    ...
    public function printHeader(ComplianceProgramStatus $status)
    {
        ...
        <script type="text/javascript">
            $(function() {
                ...
                $('.headerRow-footer').after('<tr class="quarter_one"><td style="text-align: right;">By 12/31/2017</td><td style="text-align: center;"><?php echo $status->getAttribute('quarter_1_points') ?></td><td class="status"><img src="<?php echo $status->getAttribute('quarter_1_points') >= 125 ? '/images/lights/greenlight.gif' : '/images/lights/redlight.gif' ?>" class="light" /></td><td style="text-align: center;">125</td></tr>')
                $('.quarter_one').after('<tr class="quarter_two"><td style="text-align: right;">By 03/31/2018</td><td style="text-align: center;"><?php echo $status->getAttribute('quarter_2_points') ?></td><td class="status"><img src="<?php echo $status->getAttribute('quarter_2_points') >= 125 ? '/images/lights/greenlight.gif' : '/images/lights/redlight.gif' ?>" class="light" /></td><td style="text-align: center;">125</td></tr>')
        ...
        <p>Welcome to your summary page for the 2017-18 Wellness Rewards benefit at Village of Libertyville.</p>

        <p>You have the opportunity to earn $100 in gift cards. The deadline to complete all actions is March 31, 2018.
        </p>

        <p>
            To receive the initial $50 gift card, you MUST register with HMI, complete the annual wellness screening and
            complete the Health Power Assessment by November 17, 2017 (Section 1 below).
        </p>

        <p>
            To receive an additional $25 gift card, you MUST earn 125 or more points from key actions taken for good
            health by the December 31, 2017 Deadline.
        </p>

        <p>
            To receive the last $25 gift card, you MUST earn 125 or more points from key actions taken for good health
            by the March 31, 2018 Deadline.
        </p>
        ...
        <div style="margin-top: 20px;">
            ...
            <div style="float: left;">
                ** Sponsored Wellness Events <br /><br />
                - Wellness & Benefit Fair (10/11/2017) <br />
                - Lunch & Learn (Fall 2017) <br />
                - Lunch & Learn (Winter 2018)
            </div>
        </div>
