<?php
class Culver2017ScreeningPrinter extends ScreeningProgramReportPrinter
{
...
class Culver2017CoachingView extends GraduateFromCoachingSessionComplianceView
{
...
class Culver2017LifetimeActivityView extends CompleteActivityComplianceView
{
...
class Culver2017ComplianceProgram extends ComplianceProgram
{
    ...
    public function evaluateAndStoreOverallStatus(ComplianceProgramStatus $status)
    {
        ...
        $programForRaffle1EvaluationStatus = $status->getComplianceProgram()->getEvaluateComplianceViewStatuses($status->getUser(), '2016-08-01', '2016-12-15');
        $pointsGroupPointsForRaffle1 = $programForRaffle1EvaluationStatus->getComplianceViewGroupStatus('points')->getPoints();

        $programForRaffle2EvaluationStatus = $status->getComplianceProgram()->getEvaluateComplianceViewStatuses($status->getUser(), '2016-08-01', '2017-03-01');
        $pointsGroupPointsForRaffle2 = $programForRaffle2EvaluationStatus->getComplianceViewGroupStatus('points')->getPoints();

        $programForRaffle3EvaluationStatus = $status->getComplianceProgram()->getEvaluateComplianceViewStatuses($status->getUser(), '2016-08-01', '2017-05-01');
        $pointsGroupPointsForRaffle3 = $programForRaffle3EvaluationStatus->getComplianceViewGroupStatus('points')->getPoints();

        $programForRaffleFlexEvaluationStatus = $status->getComplianceProgram()->getEvaluateComplianceViewStatuses($status->getUser(), '2016-08-01', '2017-07-31');
    ...
    public function loadGroups()
    {
        ...
        $coreGroupEnd = '2016-12-31';

        $notSpouse = function (User $user) {
            return $user->relationship_type != Relationship::SPOUSE;
        };

        $coreGroup = new ComplianceViewGroup('core', 'All Core Actions Required by '.'December 31, 2016');
        ...
        $requiredLessons = new CompleteRequiredELearningLessonsComplianceView($programStart, '2017-02-01');
...
//        $infoView->setEvaluateCallback($notSpouse);
        ...
        $preventiveExamsView = new Culver2017LifetimeActivityView(
        ...
        $fluVaccineView = new Culver2017LifetimeActivityView(
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            $printer = new Culver2017ScreeningPrinter();
            ...
            $printer = new Culver2017ComplianceProgramReportPrinter();
        }
...
class Culver2017ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
...
//        $extraELearningFinancialStatus = $pointGroupStatus->getComplianceViewStatus('elearning_financial');
        ...
        <p></p>
        <p>Welcome to your summary page for the 2016-2017 Culver Academies Wellness Rewards.</p>
        ...
        <ol>
            ...
            <ul>
                <li>For Raffle 1 - Employees or spouses must earn 200 points by December 15, 2016.</li>
                <li>For Raffle 2 - Employees or spouses must earn 300 points by March 1, 2017</li>
                <li>For Raffle 3 - Employees or spouses must earn 400 points by May 1, 2017</li>
            </ul>
            <li>For the $200 flexible benefit credit, number 1 must be done, AND:</li>
            <ul>
                <li>Employees must earn at least 200 points through the options in the table below by July 31, 2017.</li>
            </ul>
        </ol>
