...
class Wheels2021ComplianceProgram extends ComplianceProgram
{
    public function getType(User $user)
    {
        if($this->lastType && $this->lastType['user_id'] == $user->id) {
            ...
            $type = $user->getGroupValueFromTypeName('Wheels Program Type 2021', 'Program B');
    ...
    {
        $printer = new Wheels2021ComplianceProgramReportPrinter();
    ...
    public function loadGroups()
    {
        ...
        $hraView->setName('hra');
        $hraView->setAttribute('deadline', '12/01/2022');
        ...
        $tobacco->emptyLinks();
        $tobacco->addLink(new Link('Tobacco Affidavit', '/content/wheels-tobacco-affidavit-2021'));
        $core->addComplianceView($tobacco);

        $covid = new PlaceHolderComplianceView(ComplianceStatus::NOT_COMPLIANT);
        $covid->setName('covid');
        $covid->setReportName('Submit Proof of Covid-19 Vaccine to Wheels*');
        $covid->setAttribute('deadline',  '12/01/2022');
        $core->addComplianceView($covid);
        ...
        $elearning->setAttribute('deadline',  '12/01/2022');
        ...
        $wellnessActivity->setAttribute('deadline',  '12/01/2022');
...
class Wheels2021ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        $covidStatus = $coreGroupStatus->getComplianceViewStatus('covid');
        ...
        <p>
            *As part of this requirement, you will need to be fully vaccinated for COVID-19 and <span style="font-weight: bold;">
            will need to upload your vaccination card via Workday by visiting the "Announcements" section on the home page and clicking on the
            "COVID-19 Vaccine Card" link.</span>
        </p>
        <?php endif ?>
        ...
        <table class="phipTable" border="1">
            ...
            <?php if($user->getRelationshipType() != Relationship::SPOUSE) : ?>
                <tr>
                    ...
                    <td colspan="2">
                        <?php echo $covidStatus->getComplianceView()
                            ->getReportName() ?>
                    </td>
                    <td class="center">
                        <?php echo $covidStatus->getComplianceView()->getAttribute('deadline') ?>
                    </td>
                    <td class="center">
                        <?php echo $covidStatus->getComment(); ?>
                    </td>

                    <td class="center" colspan="2">
                        <img src="<?php echo $covidStatus->getLight() ?>" class="light" />
                    </td>
            ...
            <tr>
                <td class="center"><?php echo $user->getRelationshipType() == Relationship::EMPLOYEE ? '4' : '3' ?></td>
            ...
            <?php if($type == 'Program B' && $user->getRelationshipType() != Relationship::SPOUSE) : ?>
                <tr>
