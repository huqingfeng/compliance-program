...
class TransportationRepairServices2021LearningAlternativeComplianceView extends ComplianceView
{
...
class TransportationRepairServices2021ComplianceProgram extends ComplianceProgram
{
    ...
    {
        return new TransportationRepairServices2021ComplianceProgramReportPrinter();
    }
...
//        $coreGroup->setPointsRequiredForCompliance(50);
        ...
        $screeningView->setAttribute('goal', '09/27/21');
        $screeningView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($programStart, $programEnd) {
            $view = $status->getComplianceView();

            $screeningView = new CompleteScreeningComplianceView($programStart, '2021-12-31');
            $screeningView->setComplianceViewGroup($view->getComplianceViewGroup());
            $screeningView->setName('alternative_'.$view->getName());

            $screeningStatus = $screeningView->getStatus($user);

            if($screeningStatus->getStatus() ==  ComplianceStatus::COMPLIANT && $status->getStatus() == ComplianceStatus::NOT_COMPLIANT) {
                $status->setComment($screeningStatus->getComment());
            }
        });
        ...
        $hraView->setAttribute('goal', '09/27/21');
        $hraView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($programStart, $programEnd) {
            $view = $status->getComplianceView();

            $hraView = new CompleteHRAComplianceView($programStart, '2021-12-31');
            $hraView->setComplianceViewGroup($view->getComplianceViewGroup());
            $hraView->setName('alternative_'.$view->getName());

            $hraStatus = $hraView->getStatus($user);

            if($hraStatus->getStatus() ==  ComplianceStatus::COMPLIANT && $status->getStatus() == ComplianceStatus::NOT_COMPLIANT) {
                $status->setComment($hraStatus->getComment());
            }
        });
        ...
        $cotinineView->setAttribute('goal', 'Negative or 5 lessons');
        ...
        $lastYearStart = '2020-01-01';
        $lastYearEnd = '2020-11-11';
        ...
        $bmiView->setAttribute('goal', '≤29 or 2 less from 2020 or 3 lessons');
        ...
        $bmiView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($programStart, $programEnd, $lastYearStart, $lastYearEnd) {
            ...
            if($elearningStatus->getStatus() == ComplianceStatus::COMPLIANT
                && $status->getAttribute('has_result')
                && !$status->isCompliant()) {
                $status->setStatus(ComplianceStatus::COMPLIANT);
        ...
        $ldlView->setAttribute('goal', '≤130 or 10 less from 2020 or 3 lessons');
        ...
        $trigView->setAttribute('goal', '≤150 or 15 less from 2020 or 3 lessons');
        ...
        $formView->setReportName('4. Or, if needed, turn-In a Completed Alternate Qualification Form <a href="/resources/10681/Transportation_Repair&Services_2021_AQF_082521.pdf">Click here</a> to	download the form and details.');
...
class TransportationRepairServices2021ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
            ...
            <table id="results">
                <tbody>
                    ...
                    <tr class="headerRow">
                        ...
                        <th colspan="2">Goal by 10/25/21</th>
