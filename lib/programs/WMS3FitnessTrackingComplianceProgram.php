...
if ( sfConfig::get('app_wms2') ) {
    header("location: /content/fitness");
    die;
}

class WMS3FitnessTrackingComplianceProgram extends ComplianceProgram
{
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        $client = $_GET['clientId'] ?? "chp";
        $programId = $_GET['id'] ?? "";
        $wms1Id = $_GET['wms1Id'] ?? $_user->id;
        $wms2Id = $_GET['wms2AccountId'] ?? $_user->wms2_account_id;
        $device = isset($_GET['device']) ? "&device=" . $_GET['device'] : '';
        $prefix = $_GET['prefix'] ?? "";
        ...
        $forced = $_GET['user_forced'] ?? false;
        $app = strpos($_SERVER['HTTP_USER_AGENT'], 'HPN_ANDROID_APP') === false ? 'false' : 'true';
        
        if(isset($_GET['default_device_id'])) {
            $default_device_id = $_GET['default_device_id'];
            $participant_id = $_GET['participant_id'];
            $url = $site."/wms3/public/fitnesstracking?bypass=true&default_device_id=". $default_device_id."&user_id=" . $_user->getId()."&method=" . $method."&participant_id=" . $participant_id;
            echo $url;
        } else {
            $url = $site."/wms3/public/fitnesstracking?bypass=true&clientId=$client&token=$token&method=$method".$dataString."&programId=".$programId."&wms1Id=".$wms1Id."&wms2AccountId=".$wms2Id."&user_forced=".$forced.'&prefix='.$prefix.''.$device.'&android_app='.$app;    
            if(isset($_GET['fitbit_error'])) {
                $url.='&fitbit_error=1';
            }
        }
...
//        $totalCholesterolView->setAttribute('_screening_printer_hack', 8);
        ...
        $nav->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use($programStartDate, $programEndDate) {
            ...
            foreach($shadowInsurancePlanTypes as $shadowInsurancePlanType) {
                if(in_array($shadowInsurancePlanType, WMS3FitnessTrackingComplianceProgram::$historicalPpoType)) {
                    $qualified = true;
                }
            }
...
class WMS3FitnessTrackingComplianceProgramReportPrinter extends BasicComplianceProgramReportPrinter
{
