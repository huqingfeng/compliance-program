...
class Bepex20112012ScreeningTestPrinter extends ScreeningProgramReportPrinter
{
...
class Bepex20112012Printer extends BasicComplianceProgramReportPrinter
{
    public function printHeader(ComplianceProgramStatus $status)
    {
        ...
        $this->tableHeaders['total_status'] = 'Status of 1ABCD + ≥ '.Bepex20112012ComplianceProgram::POINTS_REQUIRED.' points as of:';
    ...
    <ol>
        ...
        <li>Earn 300 or more points from key screening results and key actions taken for good well-being.</li>
    </ol>
    <p>
        Employees who complete the required core actions (some by November 30, 2011), and earn 300 or more
        points from key screening results and other available actions by September 30, 2012:
    </p>
    <ol type="A">
        <li>
            Will pre-qualify their choice of the following rewards in 2013:
            <ul>
                <li>A health plan premium contribution credit of $20, $30 or $40/month (based on points earned) OR</li>
                <li>8, 12 or 16 well-being time off hours (based on points earned).</li>
            </ul>
        </li>
    </ol>
    <p><a href="/content/1094">Click here to learn more about the 2011-2012 rewards program, the related actions and
        other details.</a></p>
...
class Bepex20112012ComplianceProgram extends ComplianceProgram
{
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            $printer = new Bepex20112012ScreeningTestPrinter();
            ...
            $printer = new Bepex20112012Printer();
        }
    ...
    public function loadGroups()
    {
        ...
        $survey->setAttribute('report_name_link', '/content/1094#1cSurvey');
        $survey->setName('survey');
        //$survey->addLink(new Link('Available in July 2012', '#'));
        $survey->addLink(new FakeLink('Available in July 2012', '#'));

        $core->addComplianceView($survey);

        $wisehealth = new PlaceHolderComplianceView(ComplianceStatus::NOT_COMPLIANT);
        $wisehealth->setReportName('Complete the WiseHealth&trade; Workshop');
        $wisehealth->setName('wisehealth');
        $wisehealth->setAttribute('report_name_link', '/content/1094#1dworkshop');
        //$wisehealth->addLink(new Link('Available in March 2012', '#'));
        $wisehealth->addLink(new FakeLink('Available in August 2012', '#'));
        ...
        $coach->setName('onsite_coaching');
        $coach->setReportName('Complete Phone Health Coaching Sessions');
        $coach->setAttribute('report_name_link', '/content/1094#2allcoach');
        $coach->addLink(new Link('Information & Sign-Up', '/content/8733'));
        $coach->setComplianceStatusPointMapper(new ComplianceStatusPointMapper(40, 0, 0, 0));
        ...
        $elearn->setAttribute('report_name_link', '/content/1094#2alleLearn');
        ...
        $sem->setAttribute('report_name_link', '/content/1094#2allBewellSem');
        ...
        $carGoal->setAttribute('report_name_link', '/content/1094#2CarGoal');
        ...
        $carSem->setReportName('Complete an Online Career-Related Seminar');
        $carSem->setAttribute('report_name_link', '/content/1094#2CarOnlineSem');
        ...
        $budget->setReportName('Complete a Household Budget');
        $budget->setAttribute('report_name_link', '/content/1094#2FinBudg');
        $budget->setMaximumNumberOfPoints(10);
        ...
        $pBudget->setAttribute('report_name_link', '/content/1094#2FinOnlineSem');
        ...
        $bwellEvent->setAttribute('report_name_link', '/content/1094#2BewellEvents');
        ...
        $qualityTime->setMaximumNumberOfPoints(30);
        $qualityTime->setReportName('Spend Quality Time with Your Loved Ones');
        $qualityTime->setAttribute('report_name_link', '/content/1094#2SocQualTime');
        ...
        $spouseSurvey->setReportName('Have Your Spouse Complete the Health Assessment and Onsite Health Screening');
        $spouseSurvey->setAttribute('report_name_link', '/content/1094#2SocSpouse');
