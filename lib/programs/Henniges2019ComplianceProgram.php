...
class Henniges2019CompleteScreeningComplianceView extends CompleteScreeningComplianceView
{
...
class Henniges2019ComplianceProgram extends ComplianceProgram
{
    ...
    {
        return new Henniges2019WMS2Printer();
    }
    ...
    public function loadGroups()
    {
        ...
        $screeningStart = '2018-10-01';

        $tobaccoEnd = "2019-10-01";

        $preventionEventGroup = new ComplianceViewGroup('required', 'Prevention Events');

        
        $hraEnd = date('Y-m-d', $this->getEndDate());
        $physicianEnd = date('Y-m-d', $this->getEndDate());
        if ($_user->client_id == 3421) {
            $hraEnd = "2019-03-31";
            $physicianEnd = "2019-09-30";
        } else if ($_user->client_id == 3748) {
            $hraEnd = "2019-09-30";
            $physicianEnd = "2019-09-30";
        }
        ...
        $screeningView = new Henniges2019CompleteScreeningComplianceView($screeningStart, $physicianEnd);
        ...
        $screeningView->setAttribute('deadline',  date("m/d/Y", strtotime($physicianEnd)));
        $preventionEventGroup->addComplianceView($screeningView);

        $cotinineView = new ComplyWithTobaccoUseScreeningTestComplianceView($programStart, $programEnd);
        ...
        $cotinineView->addLink(new Link('LivingFree (for tobacco users only)', '/search-learn/lifestyle-management/content/12088?filter=livingfree'));
        $cotinineView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            ...
            if ($client_id != 3421 || ($client_id == 3421) && $relationship != 0) {
        ...
        $bloodPressureView = new ComplyWithBloodPressureScreeningTestComplianceView($screeningStart, $physicianEnd);
        ...
        $cholesterolView = new ComplyWithTotalCholesterolScreeningTestComplianceView($screeningStart, $physicianEnd);
        ...
        $hdlView = new ComplyWithHDLScreeningTestComplianceView($screeningStart, $physicianEnd);
        $hdlView->setComplianceStatusPointMapper(new ComplianceStatusPointMapper(1, 0, 0, 0));
        $biometricsGroup->addComplianceView($hdlView);

        $ldlView = new ComplyWithLDLScreeningTestComplianceView($screeningStart, $physicianEnd);
        $ldlView->setComplianceStatusPointMapper(new ComplianceStatusPointMapper(1, 0, 0, 0));
        $biometricsGroup->addComplianceView($ldlView);

        $triglView = new ComplyWithTriglyceridesScreeningTestComplianceView($screeningStart, $physicianEnd);
        ...
        $glucoseView = new ComplyWithGlucoseScreeningTestComplianceView($screeningStart, $physicianEnd);
        ...
        $heightView = new ComplyWithHeightScreeningTestComplianceView($screeningStart, $physicianEnd);
        ...
        $weightView = new ComplyWithWeightScreeningTestComplianceView($screeningStart, $physicianEnd);
        ...
        $bodyFatBMIView = new GVT2015ComplyWithBodyFatBMIScreeningTestComplianceView($screeningStart, $physicianEnd);
        $bodyFatBMIView->setIndicateSelfReportedResults(false);
        $bodyFatBMIView->setBmiView($this->getBmiView($screeningStart, $programEnd));
        $bodyFatBMIView->setBodyFatView($this->getBodyFatView($screeningStart, $programEnd));
...
class Henniges2019WMS2Printer implements ComplianceProgramReportPrinter
{
                    ...
                    <div class="row">
                        <div class="col-md-12">
                            <?php if($user->getClientId() == 3748) : ?>
                                ...
                                <p>Click on <a href="/compliance/henniges-automotive-2019/program-overview/content/henniges_program_overview">Program Overview</a> link to view your incentive details and requirements.</p>

                                <p>The report card outlines your requirements and will help you keep track of your progress throughout the program.
                                    If you choose not to complete the requirements, you will not be eligible for a prize.</p>
