...
class RJF2011ExceedLearningComplianceView extends CompleteActivityComplianceView
{
    /**
...
class RJFWorkWithDoctor2011ComplianceView extends PlaceHolderComplianceView
{
}

class RJF2011ComplianceProgram extends ComplianceProgram
{
    public function loadGroups()
    {
        ...
        $screeningView->emptyLinks()->addLink(new Link('Sign-Up', '/content/1051'));
        $coreGroup->addComplianceView($screeningView);

        $hraView = new CompleteHRAComplianceView($programStart, $programEnd);
        $hraView->setName('complete_hra');
        $hraView->emptyLinks()->addLink(new Link('Take HRA', '/content/989'));
...
//
        ...
        $doctorView = new RJFWorkWithDoctor2011ComplianceView(ComplianceStatus::NOT_COMPLIANT);
        ...
        $mindfulActivityView = new MinutesBasedMindfulActivityComplianceView($programStart, $programEnd, 20);
        ...
        $learningOrgRequirementsView = new RJF2011ExceedLearningComplianceView($programStart, $programEnd, 45, 5);
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            ...
            $printer = new RJF2011ComplianceProgramReportPrinter();
        }
...
class RJF2011ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        $learningStatus = $pointGroupStatus->getComplianceViewStatus('learning_requirements');
        $stretchStatus = $pointGroupStatus->getComplianceViewStatus('brain_break');
        ?>
    <style type="text/css">
    ...
    <div class="pageHeading">2011 Wellness Rewards/To-Dos Summary & Action Center</div>

    <p><strong>Hello <?php echo $status->getUser()->getFullName(); ?>,</strong></p>

    <p>
        Welcome to your summary page for the 2011 Wellness Rewards program. Employees meeting all criteria will each
        receive $39.58/month in reduced health insurance premiums ($475/year). To receive the incentives, eligible
        employees MUST take action and meet all criteria below by November 30, 2011:
    </p>
    ...
    <ol>
        ...
        <li>
            Earn 100 or more points from key screening results and key actions taken for good health.
        </li>
    </ol>

    <p>
        Recognizing that being well is about more than being physically fit, the 2011 Wellness Rewards program
        ...
        continue to focus on "total wellbeing" awareness in 2011.
    </p>

    <div class="pageHeading">
        <a href="/content/1094">
            Click here for the 2011 Wellness Rewards Criteria Details
        </a>.
    </div>
    ...
    <tr>
        <td><a href="/content/1094#2kMA">L. <?php echo $learningStatus->getComplianceView()->getReportName() ?></a></td>
        <td class="center"><?php echo $learningStatus->getPoints(); ?></td>
        <td class="center"><?php echo $learningStatus->getComplianceView()->getMaximumNumberOfPoints(); ?></td>
        <td class="links">
            <?php
            foreach($learningStatus->getComplianceView()->getLinks() as $link) {
                echo $link->getHTML();
            }
            ?>
        </td>
    ...
    <tr>
        <td><a href="/content/1094#2kMA">M. <?php echo $stretchStatus->getComplianceView()->getReportName() ?></a></td>
        <td class="center"><?php echo $stretchStatus->getPoints(); ?></td>
        <td class="center"><?php echo $stretchStatus->getComplianceView()->getMaximumNumberOfPoints(); ?></td>
        <td class="links">
            <?php
            foreach($stretchStatus->getComplianceView()->getLinks() as $link) {
                echo $link->getHTML();
            }
            ?>
        </td>
    ...
    <tr>
        ...
        <td class="center">100 or more points + 1AB done</td>
    </tr>
    </tbody>
    </table>
    <?php
