...
class IntegrityAerospaceGroup2015BFMapper extends ComplianceStatusPointMapper
{
...
class IntegrityAerospaceGroup2015ComplyWithBodyFatBMIScreeningTestComplianceView extends ComplyWithBodyFatBMIScreeningTestComplianceView
{
...
class IntegrityAerospaceGroup2015ComplianceProgram extends ComplianceProgram
{
    ...
    {
        $printer = new IntegrityAerospaceGroup2015Printer();
    ...
    public function loadGroups()
    {
        ...
        $hraView = new CompleteHRAComplianceView($programStart, '2014-11-30');
        $hraView->setReportName('Health Risk Assessment(HRA)');
        $preventionEventGroup->addComplianceView($hraView);

        $screeningView = new CompleteScreeningComplianceView($programStart, '2014-11-30');
        ...
        $bodyFatBMIView = new IntegrityAerospaceGroup2015ComplyWithBodyFatBMIScreeningTestComplianceView($programStart, $programEnd);
        ...
        $bodyFatBMIView->setComplianceStatusPointMapper(new IntegrityAerospaceGroup2015BFMapper());
    ...
    protected function evaluateAndStoreOverallStatus(ComplianceProgramStatus $status)
    {
        ...
        if($status->getAttribute('total_points_ignoring_alternatives') === null) {
            $status->setAttribute('total_points_ignoring_alternatives', $totalPoints - $extraPoints);
        }
    }
...
class IntegrityAerospaceGroup2015Printer extends CHPStatusBasedComplianceProgramReportPrinter
{
    ...
    public function printClientNote()
    {
        ...
        <p style="margin-top:20px;font-size:smaller;">**If contacted for coaching, employees and spouses must complete two coaching calls by May 30, 2015, or will lose incentive effective June 01, 2015.</p>


        <p style="margin-top:20px;font-size:smaller;">Pregnant? Are you pregnant or 6 months postpartum? You may complete a <a href="/resources/5019/IAG-Pregnancy-Exception-Form-2014.082114.pdf">Pregnancy Exception Form </a> and submit it to Circle Wellness to obtain credit for the body measurements (BMI and Body Fat).</p>
...
{
    $noAlternativesPoints = $this->status->getAttribute('total_points_ignoring_alternatives');
    ...
    <script type="text/javascript">
        $(function() {
            ...
            $('.headerRow.totalRow th').first().html(
                'Totals:<br/><br/>Without Alternatives:'
            );

            $('.headerRow.totalRow td:eq(2)').first().append(
                '<?php echo "<br/><br/>$noAlternativesPoints" ?>'
            );
        });
    </script>
    <style type="text/css">
    ...
    <p>
        Welcome to the 2015 IAG Wellness Program. In order to qualify for the $600 incentive in 2015, <strong>employees</strong> who are on
        the medical plan and are participating in wellness must meet 3 objectives to qualify for the 2015 incentive (unless
        ...
        for the 2015 incentive (unless contacted for health coaching**).
    </p>

    <p>
    <ol>
        <li>
            Complete an online Health Risk Assessment by <strong>November 30, 2014</strong>.
        </li>
        <li>
            Complete an onsite Health Screening by <strong>November 30, 2014.*</strong>
        </li>
        <li>
