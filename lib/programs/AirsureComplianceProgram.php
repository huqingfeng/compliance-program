...
class CompleteMoreThanFourELearningLessonsComplianceView extends CompleteELearningLessonsComplianceView
{
    ...
    /**
     * Airsure's elearning requirements have been tweaked slightly. The main requirement
     * gives points for completing any four lessons. After that, they get one point per lesson
     * in a separate view. So, we are subtracting 4 from points if points >= 4, else setting
     * points to 0.
     */
    public function getStatus(User $user)
    {
        $status = parent::getStatus($user);
        $currentPoints = $status->getPoints();
        $pointsPerLesson = $this->getPointsPerLesson();

        if($pointsPerLesson > 0) {
            $numberCompleted = $currentPoints / $pointsPerLesson;
            if($numberCompleted > 4) {
                $status->setPoints(($numberCompleted - 4) * $pointsPerLesson);
            } else {
                $status->setPoints(0);
            }
        }
...
class AirsureComplianceProgram extends ComplianceProgram
{
    public function loadGroups()
    {
        ...
        $coreGroupEnd = strtotime('2010-02-28');

        // Build the core group
        $coreGroup = new ComplianceViewGroup('core', 'Core Actions Required by '.date('F d, Y', $coreGroupEnd));
        ...
        $elearningRequiredView = new CompleteELearningLessonsComplianceView($programStart, $coreGroupEnd, null, 4);
        $elearningRequiredView->setName('complete_elearning_first_four');
        $coreGroup->addComplianceView($elearningRequiredView);

        $contactInformationView = new UpdateContactInformationComplianceView($programStart, $coreGroupEnd);
        $contactInformationView->setReportName('Personal Contact Info Up-to-Date');
        $coreGroup->addComplianceView($contactInformationView);

        $doctorInformationView = new UpdateDoctorInformationComplianceView($programStart, $coreGroupEnd);
        ...
        $workWithHealthCoachView->addLink(new Link('<br />Doctor Form', '/resources/1975/Van Gilder VG incentive DCM note from doctor 021810.pdf'));
        ...
        $additionalELearningLessonsView = new CompleteMoreThanFourELearningLessonsComplianceView($programStart, $programEnd, 1);
        $additionalELearningLessonsView->setName('complete_elearning_extra');
    ...
    public function loadEvaluators()
    {
        ...
        $firstTotal = new ProgramStatusEvaluatorComplianceView($this->cloneForEvaluation($this->getStartDate(), '2010-02-28'), array('core'), 25);
        $firstTotal->setName('first_total');
        $secondTotal = new ProgramStatusEvaluatorComplianceView($this->cloneForEvaluation($this->getStartDate(), '2010-02-28'), array('core'), 50);
        $secondTotal->setName('second_total');
        $thirdTotal = new ProgramStatusEvaluatorComplianceView($this->cloneForEvaluation($this->getStartDate(), '2010-05-31'), array('core'), 75);
        $thirdTotal->setName('third_total');
        $fourthTotal = new ProgramStatusEvaluatorComplianceView($this->cloneForEvaluation($this->getStartDate(), '2010-08-31'), array('core'), 100);
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            ...
            $printer = new AirsureComplianceProgramReportPrinter();
        }
...
class AirsureComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        $completeRequiredELearningStatus = $coreGroupStatus->getComplianceViewStatus('complete_elearning_first_four');
        ...
        $extraELearningStatus = $pointGroupStatus->getComplianceViewStatus('complete_elearning_extra');
    ...
    <p>Welcome to your summary page for the 2009-2010 Van Gilder Wellness Rewards benefit.</p>

    <p>To receive the incentive, you MUST take action and meet all criteria below:</p>

    <ol>
        <li>Complete <strong>ALL</strong> of the core required actions by February 28, 2010.</li>
    ...
    <div class="pageHeading"><a href="/content/1094">Click here for more details about the 2009-2010 Wellness Rewards
    ...
    <tr>
        <td class="right" style="font-size: 7pt;">
            All Core Actions Done on or before 02/28/2010
        </td>
        <td></td>
        <td class="center">
    ...
    <tr>
        <td style="text-align: right;">Total & Credit Status: Through 02/28/2010 =</td>
    ...
    <tr>
        <td style="text-align: right;">Through 2/28/2010 =</td>
    ...
    <tr>
        <td style="text-align: right;">Through 5/31/2010 =</td>
    ...
    <tr>
        <td style="text-align: right;">Through 8/31/2010 =</td>
