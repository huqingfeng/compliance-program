...
class UCMCTHC2018ComplianceProgram extends ComplianceProgram
{
    const APPOINTMENT_PRE_START_DATE = '2018-01-16';
    const APPOINTMENT_PRE_END_DATE = '2018-02-02';
    const APPOINTMENT_POST_START_DATE = '2018-03-27';
    const APPOINTMENT_POST_END_DATE = '2018-04-13';
    const REGISTRATION_END_DATE = '2018-02-01 23:59:59';
    const FITBIT_STEPS_DISPLAY_START_DATE = '2018-02-05';
    const BUDDY_END_DATE = '2018-01-29 23:59:59';
    ...
    {
        $actions->getUser()->setNoticeFlash('Sorry, 2018 Total Health Challenge has ended');
    ...
    public function preQuery(Doctrine_Query $query, $withViews = true)
    {
        $userIds = SelectQuery::create()
            ...
            ->on('r.user_id = u.id AND r.compliance_program_record_id = ?', array(self::UCMC_TOTAL_HEALTH_CHALLENGE_2018_RECORD_ID))
            ->leftJoin('compliance_program_record_team_users t')
            ->on('t.user_id = u.id AND t.compliance_program_record_id = ?', array(self::UCMC_TOTAL_HEALTH_CHALLENGE_2018_RECORD_ID))
            ->leftJoin('compliance_program_record_buddies b1')
            ->on('b1.first_user_id = u.id AND b1.compliance_program_record_id = ?', array(self::UCMC_TOTAL_HEALTH_CHALLENGE_2018_RECORD_ID))
            ->leftJoin('compliance_program_record_buddies b2')
            ->on('b2.second_user_id = u.id AND b2.compliance_program_record_id = ?', array(self::UCMC_TOTAL_HEALTH_CHALLENGE_2018_RECORD_ID))
    ...
    {
        return new UCMCTHC2018TeamDashboardPrinter();
    }

    public function getBuddyDashboardPrinter()
    {
        return new UCMCTHC2018BuddyDashboardPrinter();
    }

    public function getRegistrationForm()
    {
        return new UCMCTHC2018RegistrationForm();
    }

    public function getRegistrationFormPrinter()
    {
        return new UCMCTHC2018RegistrationFormPrinter();
    }

    public function getProgramReportPrinter($preferredPrinter = null)
    {
        return new UCMCTHC2018ProgramReportPrinter();
    }
    ...
    public function getAdminProgramReportPrinter()
    {
        ...
        $printer->addCallbackField('Date Of Birth', function(User $user) use ($program) {
            return $user->getAge();
        });
        ...
        $printer->addCallbackField('Registration Date', function(User $user) use ($program) {
            $jsonData = SelectQuery::create()
                ->select('form_data')
                ->from('compliance_program_record_user_registrations')
                ->where('compliance_program_record_id = ?', array($program->getID()))
                ->andWhere('user_id = ?', array($user->id))
                ->hydrateSingleScalar()
                ->execute();

            if($jsonData && ($decodedJsonData = json_decode($jsonData, true))) {
                if(isset($decodedJsonData['registration_date'])) {
                    return date('m/d/Y', strtotime($decodedJsonData['registration_date']));
                } else {
                    return null;
                }
            } else {
    ...
    public function getPreAppointmentDate(User $user, $typeId = 49)
    {
        return SelectQuery::create()
            ...
            ->andWhere(
                'date BETWEEN ? AND ?',
                array(UCMCTHC2018ComplianceProgram::APPOINTMENT_PRE_START_DATE, UCMCTHC2018ComplianceProgram::APPOINTMENT_PRE_END_DATE)
            )
    ...
    public function loadSessionParameters()
    {
        $_SESSION['manua_override_fitbit_parameters'] = array(
            'activity_id' => '288',
            'question_id' => '110',
            'start_date' => UCMCTHC2018ComplianceProgram::FITBIT_STEPS_DISPLAY_START_DATE,
            'end_date' => $this->getEndDate('Y-m-d'),
            'product_name'  => 'Total Health Challenge',
            'header_text'  => '<p><a href="/compliance_programs?id=1310">Back to My Dashboard</a></p>',
            'override' => 0
        );
    }
    ...
    public function loadGroups()
    {
        ...
        $this->options = array(
                ...
                'team_members_maximum_for_accepted_users'   => false,
                'team_members_maximum_males'     => 3,
                'require_registration'           => true,
                'force_spouse_with_employee'     => true,
                'force_spouse_with_employee_excluded_spouses'   => array(2746085, 2777188, 3089560, 2994701, 2744997, 2852602, 2894603, 2805872),
                'registration_end_date'          => UCMCTHC2018ComplianceProgram::REGISTRATION_END_DATE,
                'team_buddy_management_end_date' => UCMCTHC2018ComplianceProgram::BUDDY_END_DATE,
                'registration_redirect'          => '/content/wms2-appointment-center',
                'registration_close_redirect'    => sprintf('/compliance_programs/localAction?id=%s&local_action=registration_closed', UCMCTHC2018ComplianceProgram::UCMC_TOTAL_HEALTH_CHALLENGE_2018_RECORD_ID)
            ) + $this->options;
        ...
        $elearn = new CompleteELearningGroupSet($startDate, strtotime('2018-04-13'), 'required_thc_2018');
        ...
        $fitbitView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use ($startDate, $endDate) {
            ...
            $alternativeView = new UCMCWalkingCampaignFitbitComplianceView(UCMCTHC2018ComplianceProgram::FITBIT_STEPS_DISPLAY_START_DATE, $endDate, 70000, 10);
    ...
    public function getPostScreeningData(User $user)
    {
        if($this->postCall && $this->postCall['id'] == $user->id) {
            ...
            $data = Screening::getMergedData(
                $user,
                new \DateTime(UCMCTHC2018ComplianceProgram::APPOINTMENT_POST_START_DATE),
                new \DateTime(UCMCTHC2018ComplianceProgram::APPOINTMENT_POST_END_DATE),
                array(
...
//            $points['inches']  = $inchesPoints;
    ...
    public function executeExerciseEntry(sfActions $actions)
    {
        ...
        list($record, $recordObj) = UCMCTHC2018ComplianceProgram::thcRecord($user);
        ...
        if ($form->isValidForRequest($actions->getRequest())) {
            if(date('Y-m-d', strtotime($date)) > date('Y-m-d')) {
                ...
                $actions->redirect('/compliance_programs/localAction?id=1310&local_action=exercise_calendar');
            }
            ...
            if(($aerobicMinutes || $resistenceMinutes) && ($aerobicMinutes > 0 || $resistenceMinutes> 0)) {
                if($entryNum) {
                    if($record["{$date}_{$entryNum}_aerobic_minutes"] || $record["{$date}_{$entryNum}_resistence_minutes"]) {
                        ...
                        $actions->redirect(sprintf('/compliance_programs/localAction?id=1310&local_action=exercise_entry&day=%s', $date));
                    }
                } else {
        ...
        <div class="page-header">
            <p><a href="/compliance_programs/localAction?id=1310&local_action=exercise_calendar">Back to Calendar</a></p>
            <h3>Logging exercise information for <?php echo $dateTime->format('l m/d/Y') ?></h3>
        </div>
        <?php echo $form->renderFormTag('/compliance_programs/localAction?id=1310&local_action=exercise_entry&day='.$dateTime->format('Y-m-d')) ?>
            <ul>
                <?php echo $form ?>
                <li class="actions">
            ...
            <div class="form-actions">
                    ...
                    <a class="btn btn-default" href="/compliance_programs/localAction?id=1310&local_action=exercise_calendar">
                        Cancel
                    </a>
            </div>
        </form>
    ...
    public function executeRegistrationClosed(sfActions $actions)
    {
        ...
        if(isset($_GET['admin'])) {
            $actions->redirect(sprintf('/compliance_programs/register?id=%s&admin', $this->getComplianceProgramRecord()->getId()));
        }

        ?>
        <p class="alert alert-info">
            Sorry, sign-up period for THC is now closed.
        </p>
    ...
    public function executeExerciseCalendar(sfActions $actions)
    {
        ...
        ?>
        <p><a id="back-to-report-card" href="/compliance_programs?id=1310">Back to My Dashboard</a></p>
                    ...
                    foreach($weeks as $n => $days) {
                        ...
                        <div class="row week">
                            ...
                            <div class="col-xs-12 col-md-1 <?php echo $days[0] ? 'day' : '' ?>">
                                ...
                                <?php if($days[0]) : ?>
                                    <a href="<?php echo '/compliance_programs/localAction?id=1310&local_action=exercise_entry&day='.$days[0] ?>">
                            ...
                            <div class="col-xs-12 col-md-1 <?php echo $days[1] ? 'day' : '' ?>">
                                ...
                                <?php if($days[1]) : ?>
                                    <a href="<?php echo '/compliance_programs/localAction?id=1310&local_action=exercise_entry&day='.$days[1] ?>">
                            ...
                            <div class="col-xs-12 col-md-1 <?php echo $days[2] ? 'day' : '' ?>">
                                ...
                                <?php if($days[2]) : ?>
                                    <a href="<?php echo '/compliance_programs/localAction?id=1310&local_action=exercise_entry&day='.$days[2] ?>">
                            ...
                            <div class="col-xs-12 col-md-1 <?php echo $days[3] ? 'day' : '' ?>">
                                ...
                                <?php if($days[3]) : ?>
                                    <a href="<?php echo '/compliance_programs/localAction?id=1310&local_action=exercise_entry&day='.$days[3] ?>">
                            ...
                            <div class="col-xs-12 col-md-1 <?php echo $days[4] ? 'day' : '' ?>">
                                ...
                                <?php if($days[4]) : ?>
                                    <a href="<?php echo '/compliance_programs/localAction?id=1310&local_action=exercise_entry&day='.$days[4] ?>">
                            ...
                            <div class="col-xs-12 col-md-1 <?php echo $days[5] ? 'day' : '' ?>">
                                ...
                                <?php if($days[5]) : ?>
                                    <a href="<?php echo '/compliance_programs/localAction?id=1310&local_action=exercise_entry&day='.$days[5] ?>">
                            ...
                            <div class="col-xs-12 col-md-1 <?php echo $days[6] ? 'day' : '' ?>">
                                ...
                                <?php if($days[6]) : ?>
                                    <a href="<?php echo '/compliance_programs/localAction?id=1310&local_action=exercise_entry&day='.$days[6] ?>">
    ...
    public function executePrePostMeasurements(sfActions $actions)
    {
        ...
        <table id="pre-post" class="table table-striped">
            ...
            <tr>
                <td>Blood Pressure
                    <div style="font-size:10pt;">25 pts if post is <120/<80 or if either post is lower than pre, but neither is higher</div>
                </td>
    ...
    const UCMC_TOTAL_HEALTH_CHALLENGE_FITNESS_RECORD_TYPE = 'ucmc_thc_fitness_2018';
    const UCMC_TOTAL_HEALTH_CHALLENGE_2018_RECORD_ID = 1310;
}
...
class UCMCTHC2018ProgramReportPrinter implements ComplianceProgramReportPrinter
{
        ...
        <div class="page-header" id="ucmc-thc-header">
            <div class="row">
                    <div class="col-md-7">
                        ...
                        <p>Thank you for signing up for the 2018 Total Health Challenge.</p>

                        <p>From this dashboard below you can:</p>

                        <ul>
                            <li><a href="https://static.hpn.com/wms2/documents/clients/ucmc/UCMC_THC_2018_Exercise_Planner_Log_Only.pdf" target="_blank">Download Exercise Log</a></li>
                    ...
                    <div class="col-md-5">
                        <img src="https://static.hpn.com/wms2/images/clients/ucmc/2018_THC_Logo_2_blue.png" class="img-responsive" />

                        <br/>

                        <p>
                            <span class="text-danger">Post-Contest Measures are Apr 2 - Apr 13, 2018.</span>
                        </p>

                        <p style="font-weight: bold;">
                            <a href="https://static.hpn.com/wms2/documents/clients/ucmc/UCMC_THC_2018_Combined_List_of_Winners_060518.pdf" target="_blank">
                                Click here to see the winners of the 2018 Total Health Challenge.
                            </a>
                        </p>
                    </div>
            </div>
                ...
                <div class="panel <?php echo $exerciseClass ?>">
                    ...
                    <div class="panel-body text-center">
                        ...
                        <div class="row">
                            <div class="col-md-12">
                                &nbsp;&nbsp;
                                <a href="https://static.hpn.com/wms2/documents/clients/ucmc/UCMC_THC_2018_Exercise_Planner_Log_Only.pdf" target="_blank">Exercise Log (PDF)</a>
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-md-6">
                <div class="panel <?php echo $measurementsClass ?>">
                    ...
                    <div class="panel-body text-center">
                        ...
                        <div class="row" >
                            <div class="col-md-12">
                                <a href="/compliance_programs/localAction?id=1310&local_action=pre_post_measurements">View Details</a>
                            </div>
                        </div>
                    </div>
                </div>
...
class UCMCTHC2018TeamDashboardPrinter extends UCMCTHC2018BuddyAndTeamDashboardPrinter implements TeamDashboardPrinter
{
...
class UCMCTHC2018BuddyDashboardPrinter extends UCMCTHC2018BuddyAndTeamDashboardPrinter implements BuddyDashboardPrinter
{
...
abstract class UCMCTHC2018BuddyAndTeamDashboardPrinter
{
...
class UCMCTHC2018RegistrationForm extends BaseForm
{
    public function configure()
    {
        ...
        $locations = array(
            'UCM' => array(
                ''           => 'Select One',
                'burr_ridge_finance' => 'Burr Ridge (Finance)',
                'burr_ridge_call_center' => 'Burr Ridge (Call Center)',
                'darien' => 'Darien',
                'hyde_park'       => 'Hyde Park',
            ...
            'Ingalls' => array(
                'harvey' => 'Harvey',
                'ifcc_flossmore' => 'IFCC Flossmore',
                'ifcc_tinley' => 'IFCC Tinley',
                'ifcc_calumet_city' => 'IFCC Calumet City',
                'medcentrix_calumet_city' => 'Medcentrix Calumet City',
                'ingalls_care_center_crestwood' => 'Ingalls Care Center Crestwood',
                'ingalls_care_center_south_holland' => 'Ingalls Care Center South Holland',
                'other' => 'Other'
            )
        );
        ...
        $this->setWidgets(array(
            ...
            'registration_date' => new sfWidgetFormInputHidden(array(), array('value' => date('Y-m-d')))
        ));

        $location_validate = array_filter(array_merge(array_keys($locations['Ingalls']), array_keys($locations['UCM'])));

        $this->setValidators(array(
            'department'    => new sfValidatorChoice(array('choices' => array_keys($departments))),
            'location'      => new sfValidatorChoice(array('choices' => $location_validate)),
            'agree'         => new sfValidatorChoice(array('choices' => array(1))),
            'registration_date' => new sfValidatorString(array('required' => false))
        ));
    }
}

class UCMCTHC2018RegistrationFormPrinter implements RegistrationFormPrinter
{
    public function printForm(BaseForm $form, $url, User $user)
    {

        $record = ComplianceProgramRecordTable::getInstance()->find(1310);

        $allowRegistration = true;

        $excludedSpouses = $record->getComplianceProgram()->getOption('force_spouse_with_employee_excluded_spouses');

        if ($user->relationship_type == Relationship::SPOUSE && $user->relationshipUser->exists() && !in_array($user->id, $excludedSpouses)) {
        ...
        <img class="img img-responsive" src="https://static.hpn.com/wms2/images/clients/ucmc/2018_THC_Logo_2_blue.png" alt="Total Health Challenge" />
                        ...
                        <div class="col-md-12">
                            <?php if($user->client_id == 2251) : ?>
                                <p>By signing up for the Total Health Challenge (THC): A)
                                    For the duration of the 2018 THC (i.e., from the time I sign-up through the date
