...
class MTI2016ComplianceProgram extends ComplianceProgram
{
    public function getAdminProgramReportPrinter() 
    {
        ...
        $printer->addMultipleStatusFieldsCallback(function(ComplianceProgramStatus $status) {
            ...
            $biometricStatus = $status->getComplianceViewGroupStatus('Requirements');

            $data['Biometric Points'] = $biometricStatus->getPoints();
            ...
            foreach($quarterlyReportIds as $quarterlyReportId => $quarterName) {
                ...
                $data[sprintf('%s - Points', $quarterName)] = $quarterlyReportStatus->getPoints();
            }
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        if (sfConfig::get('app_wms2')) {
            return new MTI2016WMS2Printer();
        } else {
            $printer = new MTI2016Printer();
...
class MTI2016Printer extends CHPStatusBasedComplianceProgramReportPrinter
{
...
class MTI2016WMS2Printer implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        $biometricStatus = $status->getComplianceViewGroupStatus('Requirements');
                                ...
                                <div class="col-md-4">
                                    <div class="row">
                                        <div class="col-xs-8 col-xs-offset-2 col-md-10 col-md-offset-1 activity">
                                            <?php echo $circle(
                                                $biometricStatus->getStatus(),
                                                '<span class="circle-points">'.$biometricStatus->getPoints(). '</span><br/><br/>Biometric<br/>points'
                                            ) ?>
                                            <br/>
