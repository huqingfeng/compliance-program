...
class Wheels2017ComplianceProgram extends ComplianceProgram
{
    const POINTS = 81;
    ...
    public function getAdminProgramReportPrinter()
    {
        ...
        $printer->addMultipleStatusFieldsCallback(function(ComplianceProgramStatus $status) {
            $oldPoints = $status->getComplianceViewGroupStatus('2017_points')->getPoints();
            $newPoints = $status->getComplianceViewGroupStatus('points')->getPoints();

            return array(
                '2017_points'                    => $oldPoints,
                '2018_points'                    => $newPoints,
                '2017_2018_change'               => bcsub($newPoints, $oldPoints, 1),
                Wheels2017ComplianceProgram::POINTS.'+ in 2017, or 3+ improvement' => $status->getComplianceProgram()->isCompliantFor1C($oldPoints, $newPoints)
            );
        });
    ...
    {
        return $newPoints >= Wheels2017ComplianceProgram::POINTS || ($oldPoints > 0 && $newPoints - $oldPoints >= 3);
    }
    ...
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        ...
        if($preferredPrinter == 'ScreeningProgramReportPrinter' && $this->getActiveUser() !== null) {
            $printer = new Wheels2017ScreeningTestPrinter();
            $printer->setShowLight(true);
            $printer->setShowPoints(false);

            return $printer;
        } else {
            $printer = new Wheels2017ComplianceProgramReportPrinter();
        }
    ...
    {
        $oldProgramRecord = ComplianceProgramRecordTable::getInstance()->find(857);
        ...
        $oldPointsGroup = new ComplianceViewGroup('2017_points', '2017 Numbers');
        ...
        $oldScreeningTestViews = $this->getScreeningTestViews($screeningModel, $screeningTestAlias, '2016-07-01', '2017-08-31', new \DateTime('2015-10-31'), '', $oldProgramRecord, null);

        $core = new ComplianceViewGroup('core', 'Core actions required by 11/03/2017');

        $scrView = new CompleteScreeningComplianceView($startDate, $endDate);
        $scrView->setReportName('Complete Wellness Screening');
        $scrView->setAttribute('deadline', '11/03/2017');
        ...
        $hpaView->setAttribute('deadline', '11/03/2017');
        ...
        $alternativeWellnessView = new PlaceHolderComplianceView(ComplianceStatus::NOT_COMPLIANT);
        $alternativeWellnessView->setReportName('Alternative Wellness Activities');
        $alternativeWellnessView->setName('alternative_wellness_activities');
        $alternativeWellnessView->setAllowPointsOverride(true);
        $alternativeGroup->addComplianceView($alternativeWellnessView);
    ...
    public function evaluateAndStoreOverallStatus(ComplianceProgramStatus $status)
    {
        ...
        if($coreGroupStatus->getStatus() == ComplianceStatus::COMPLIANT
            && ($pointsGroupStatus->getPoints() <= 80.9 || $cotinineStatus->getStatus() != ComplianceStatus::COMPLIANT)) {
            $status->setAttribute('show_alternatives', true);
        } else {
            $status->setAttribute('show_alternatives', false);
        }
...
class Wheels2017ComplianceProgramReportPrinter implements ComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ...
        $alternativeStatus = $status->getComplianceViewStatus('alternative_wellness_activities');


        $newBiometricPoints = $pointGroupStatus->getPoints();
        $oldBiometricPoints = $status->getComplianceViewGroupStatus('2017_points')->getPoints();
        ?>
        <style type="text/css">
        ...
        <p>Stay tuned for exciting wellness program changes in 2018!</p>
        ...
        <table class="phipTable" border="1">
            ...
            <tr>
                <td colspan="2" class="left">
                    ...
                    <div style="text-align:right;">A. Total Biometric Points for 2017 = </div>
                    <div style="text-align:right;">B. Total Points from 2016 = </div>
                    <div style="text-align:right;">-------------------------------------------- </div>
                    <div style="text-align:right;">C. Point Increase or (Decrease) in 2017 =</div>
                </td>
                <td class="center">
            ...
            <?php if($status->getAttribute('show_alternatives')) : ?>
            <tr>
                <td style="color:#4169E1;font-size:10pt; text-align: center"><?php echo $alternativeStatus->getComplianceView()
                            ->getReportName() ?></td>
                <td class="center">
                    <?php echo $alternativeStatus->getComment(); ?>
                </td>
                <td class="center">
                    <img src="<?php echo $alternativeStatus->getLight(); ?>" class="light"/>
                </td>
                <td class="center">
...
class Wheels2017ScreeningTestPrinter extends ScreeningProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        ?>
