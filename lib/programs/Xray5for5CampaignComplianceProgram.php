...
define('XRAY_FIVE_FOR_FIVE_CAMPAIGN', sfConfig::get('mod_legacy_chp_five_for_five_campaign_data_type', 'xray_five_for_five_campaign_2017'));
define('XRAY_FIVE_FOR_FIVE_CAMPAIGN_START_DATE', sfConfig::get('mod_legacy_chp_five_for_five_campaign_start_date', '2017-07-31'));
define('XRAY_FIVE_FOR_FIVE_CAMPAIGN_END_DATE', sfConfig::get('mod_legacy_chp_five_for_five_campaign_end_date', '2017-09-03'));

class Xray5For5CampaignWeeklyPointsComplianceView extends ComplianceView
{
    ...
    {
        $record = $user->getNewestDataRecord(XRAY_FIVE_FOR_FIVE_CAMPAIGN, true);
...
class Xray5For5CampaignAdminPrinter extends BasicComplianceProgramAdminReportPrinter
{
...
class Xray5For5CampaignComplianceProgram extends ComplianceProgram
{
    public function getAdminProgramReportPrinter()
    {
        $record = $this->getComplianceProgramRecord();

        return new Xray5For5CampaignAdminPrinter($record);
    }

    public function getTeamDashboardPrinter()
    {
        return new Xray5For5CampaignTeamDashboardPrinter();
    }

    public function getTeamData(sfActions $actions)
    {
        $activeProgramRecord = ComplianceProgramRecordTable::getInstance()->find(1266);

        $teams = $activeProgramRecord->getTeams();

        $allUsersPoints = XrayFiveForFiveCampaignCommitmentForm::getAllUsersPoints($actions);
    ...
    {
        return new Xray5For5CampaignBuddyDashboardPrinter();
    }

    public function getProgramReportPrinter($preferredPrinter = null)
    {
        return new Xray5For5CampaignProgramReportPrinter();
    }
    ...
    public function loadGroups()
    {
        ...
        $week1View = new Xray5For5CampaignWeeklyPointsComplianceView('week1');
        $week1View->setReportName('Week 1 Points');
        $week1View->setName('week1_points');
        $week1View->addLink(new Link('Log fruits & vegetables', '/compliance_programs/localAction?id=1266&local_action=5for5_dashboard'));
        $operations->addComplianceView($week1View);

        $week2View = new Xray5For5CampaignWeeklyPointsComplianceView('week2');
        $week2View->setReportName('Week 2 Points');
        $week2View->setName('week2_points');
        $week2View->addLink(new Link('Log fruits & vegetables', '/compliance_programs/localAction?id=1266&local_action=5for5_dashboard'));
        $operations->addComplianceView($week2View);

        $week3View = new Xray5For5CampaignWeeklyPointsComplianceView('week3');
        $week3View->setReportName('Week 3 Points');
        $week3View->setName('week3_points');
        $week3View->addLink(new Link('Log fruits & vegetables', '/compliance_programs/localAction?id=1266&local_action=5for5_dashboard'));
        $operations->addComplianceView($week3View);

        $week4View = new Xray5For5CampaignWeeklyPointsComplianceView('week4');
        $week4View->setReportName('Week 4 Points');
        $week4View->setName('week4_points');
        $week4View->addLink(new Link('Log fruits & vegetables', '/compliance_programs/localAction?id=1266&local_action=5for5_dashboard'));
        $operations->addComplianceView($week4View);

        $week5View = new Xray5For5CampaignWeeklyPointsComplianceView('week5');
        $week5View->setReportName('Week 5 Points');
        $week5View->setName('week5_points');
        $week5View->addLink(new Link('Log fruits & vegetables', '/compliance_programs/localAction?id=1266&local_action=5for5_dashboard'));
...
EOT
    ...
    public function execute5For5Dashboard(sfActions $actions)
    {
        ...
        $form = new XrayFiveForFiveCampaignCommitmentForm(array(), array('user' => $user));

        $record = $user->getNewestDataRecord(XRAY_FIVE_FOR_FIVE_CAMPAIGN, true);

        $points = XrayFiveForFiveCampaignCommitmentForm::getPoints($record);
        $clientPoints = XrayFiveForFiveCampaignCommitmentForm::getClientPoints($actions);
                ...
                <div class="tab-pane active" id="record_your_food">
                    <div>
                        ...
                        <div style="margin-top: 35px; font-size: 11pt;">
                            <br /><br /><a href="/compliance_programs?id=1266">Back to my dashboard</a>
                        </div>
                    ...
                    <?php echo $form->renderFormTag('/compliance_programs/localAction?id=1266&local_action=save_your_fruits') ?>
                    <?php echo $form->renderGlobalErrors() ?>
                    <?php echo $form->renderHiddenFields() ?>
                    <div  style="clear:both;">
                        <?php foreach(XrayFiveForFiveCampaignCommitmentForm::getWeeks($user) as $week => $weekName) :?>
                            <?php $startEndDates = XrayFiveForFiveCampaignCommitmentForm::getWeekStartEndDates(); ?>
    <!--                        --><?php //if($week != 'week1' && isset($startEndDates[$week]['start_date']) && $startEndDates[$week]['start_date'] > date('Y-m-d')) continue; ?>
                            ...
                            <div style="padding-top:15px;" id="<?php echo $week ?>">
                                ...
                                <div class="week_content">
                                    ...
                                    <?php foreach(XrayFiveForFiveCampaignCommitmentForm::getDays() as $day => $dayName) : ?>
                                        <span class="label label-success" style="line-height:20px; padding-top: 6px;"><?php echo $dayName ?></span>

                                        <?php foreach(XrayFiveForFiveCampaignCommitmentForm::getFruitTypes() as $fruitType) :?>
                ...
                <div class="tab-pane" id="smoothie_basics">
                    ...
                    <div style="margin-bottom: 20px;">
                        ...
                        <img src="/resources/7253/Xray_Smoothies_5weeks_footer.png"  style="width: 100%;"/>
                    </div>
    ...
    public function executeSaveYourFruits($actions)
    {
        ...
        $weekStartEndDates = XrayFiveForFiveCampaignCommitmentForm::getWeekStartEndDates();
                    ...
                    if(!empty($value)) {
                        foreach($weekStartEndDates as $week => $startEndDate) {
                            if(strpos($field, $week) !== false
                                ...
                                $actions->redirect('/compliance_programs/localAction?id=1266&local_action=5for5_dashboard#record_your_food');
                            }
                        }
        ...
        $actions->redirect('/compliance_programs/localAction?id=1266&local_action=5for5_dashboard');
    }
...
class Xray5For5CampaignProgramReportPrinter implements ComplianceProgramReportPrinter
{
...
class Xray5For5CampaignTeamDashboardPrinter extends Xray5For5CampaignBuddyAndTeamDashboardPrinter implements TeamDashboardPrinter
{
...
class Xray5For5CampaignBuddyDashboardPrinter extends Xray5For5CampaignBuddyAndTeamDashboardPrinter implements BuddyDashboardPrinter
{
...
abstract class Xray5For5CampaignBuddyAndTeamDashboardPrinter
{
...
class XrayFiveForFiveCampaignCommitmentForm extends BaseForm
{
    public function configure()
    {
        ...
        if($record->exists()){
            foreach($record->getAllDataFields() as $field) {
                if(XRAY_FIVE_FOR_FIVE_CAMPAIGN_START_DATE <= date('Y-m-d', strtotime($field->getField('creation_date')))
                    && date('Y-m-d', strtotime($field->getField('creation_date'))) <= XRAY_FIVE_FOR_FIVE_CAMPAIGN_END_DATE) {
        ...
        $weeks = array_keys(XrayFiveForFiveCampaignCommitmentForm::getWeeks($user));
        $days = array_keys(XrayFiveForFiveCampaignCommitmentForm::getDays());
        $fruitTypes = XrayFiveForFiveCampaignCommitmentForm::getFruitTypes();


        $widgets = array();
        $validators = array();

        $fruitChoices = XrayFiveForFiveCampaignCommitmentForm::getFruitChoices();
    ...
    public static function getPoints($record, $startDate = false, $endDate = false)
    {
        ...
        foreach($weeks as $week) {
            ...
            if(isset($weekStartEndDates[$week]['start_date']) && isset($weekStartEndDates[$week]['end_date'])) {
                $completedLessons = XrayFiveForFiveCampaignCommitmentForm::getCompletedLessons($user, $weekStartEndDates[$week]['start_date'], $weekStartEndDates[$week]['end_date']);
    ...
    public static function getAllUsersPoints($actions)
    {
        ...
        $records = UserDataRecord::getRecordsForAllUsers(XRAY_FIVE_FOR_FIVE_CAMPAIGN, $client, true);
