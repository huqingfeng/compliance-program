<?php
  ...
  class SandCElectric2022CompleteScreeningComplianceView extends CompleteScreeningComplianceView {
  ...
  class SandCElectric2022ComplianceProgram extends ComplianceProgram {
    ...
    public function loadGroups() {
      ...
      $ampSignup->setAttribute('deadline', '03/31/2022');
      ...
      $screening = new SandCElectric2022CompleteScreeningComplianceView($startDate, $endDate);
      $screening->setComplianceStatusPointMapper(new ComplianceStatusPointMapper(1, 0, 0, 0));
      $screening->setAttribute('deadline', '03/31/2022');
      $screening->setReportName('<b>B.</b> Complete Biometric Screening');
      $screening->emptyLinks();
      $screening->setName('screening');
      $screening->addLink(new Link('Screening Options', '/content/appointments'));
      ...
      $hra->setAttribute('deadline', '06/30/2022');
      $hra->emptyLinks();
      $hra->setName('hra');
      ...
      $hra->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
        ...
        if($surveyStatus->getStatus() == ComplianceStatus::COMPLIANT
          && date('Y-m-d', strtotime($surveyStatus->getComment())) >= '2021-10-01')
      ...
      $focusClass = $this->getTempView(
        'focus_class',
        '<b>E.</b> 1 Focus Class',
        '09/30/2022'
      );
      $focusClass->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
      ...
      $req->addComplianceView($focusClass);
      ...
      $attestationForm = $this->getTempView(
        'attestation_form',
        'BCBS Program : Submission of the completed Physician Attestation Form 1087 that indicates you are managing your condition with the help of your PCP or other appropriate medical professional',
        '09/30/2022',
    ...
    public function getProgramReportPrinter($preferredPrinter = null) {
      return new SandCElectric2022ComplianceProgramReportPrinter();
    }
    ...
    public function getAdminProgramReportPrinter() {
      ...
      $printer->addCallbackField('Weigh In Screening Date', function (User $user) {
        $data = SandCElectric2022ComplianceProgram::getScreeningData($user, '2021-10-01', '2022-09-30');
    ...
    public static function getScreeningData(User $user, $startDate = '2021-10-01', $endDate = '2022-09-30') {
  ...
  class SandCElectric2022ComplianceProgramReportPrinter extends BasicComplianceProgramReportPrinter {
    public function printReport(ComplianceProgramStatus $status) {
      ...
      $fifth_activity_name = count($track_views) > 1 ? '<b>E.</b> Next Steps: PICK ONE of the action items below: <br />' : '';
