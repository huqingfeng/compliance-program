...
class NSK2014ComplianceProgram extends ComplianceProgram
{
    public function getProgramReportPrinter($preferredPrinter = null)
    {
        $printer = new NSK2014Printer();
    ...
    public function getElearningLessonEndDate(User $user)
    {
        $screenings = SelectQuery::create()
            ->select('s.date')
            ->from('screening s')
            ->innerJoin('hra h')
            ->on('s.user_id = h.user_id')
            ->where('s.date BETWEEN ? and ?', array('2014-04-01', '2014-12-31'))
            ->andWhere('h.date BETWEEN ? AND ?', array('2014-04-01', '2014-12-31'))
            ->andWhere('s.user_id = ?', array($user->id))
            ->andWhere("s.cholesterol != ''")
            ->orderBy('s.date DESC')
            ->execute();

        foreach($screenings as $screening) {
            return date('Y-m-d', strtotime('+35 days',strtotime($screening['date'])));
        }
    ...
    protected function configureViewForElearningAlternative(ComplianceView $view, $alias, $allAliases)
    {
        ...
        $view->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use(&$callCache, $alias, $allAliases, $userIdsToIgnore) {
            ...
            if($status->getAttribute('has_result') && $status->getPoints() < $view->getMaximumNumberOfPoints()) {
                ...
                $program = $view->getComplianceViewGroup()->getComplianceProgram();
                $elearningEndDate = $program->getElearningLessonEndDate($user);

                $elearningView = new CompleteELearningGroupSet($view->getStartDate(), $elearningEndDate, $aliasesToUse);
    ...
    protected function evaluateAndStoreOverallStatus(ComplianceProgramStatus $status)
    {
        ...
        $priorStatus = $status->getComplianceProgram()->getPriorYearBiometricsComplianceViewStatuses($status->getUser(), '2013-02-01', '2014-03-01');
        $priorPriorStatus = $status->getComplianceProgram()->getPriorYearBiometricsComplianceViewStatuses($status->getUser(), '2012-05-15', '2012-12-31');
        ...
        foreach($status->getComplianceViewGroupStatuses() as $groupStatus) {
            foreach($groupStatus->getComplianceViewStatuses() as $viewStatus) {
                ...
                if($viewStatus->getComplianceView()->getName() == 'bf_bmi') {
                    if($viewStatus->getPoints() < 4) $allCompliant = false;
                } else {
...
class NSK2014Printer extends CHPStatusBasedComplianceProgramReportPrinter
{
    ...
    public function printClientMessage()
    {
        ...
        <p>Below is your 2014 Incentive Report Card.</p>
        <p><a href="/content/program_overview">CLICK HERE </a> to view the full details of the 2014 program.</p>
        <p><span style="color:red">IF YOU HAVE “ALTERNATIVE” LINKS IN THE BIOMETRIC SECTION OF YOUR REPORT CARD, YOU MUST COMPLETE
        ...
        <script type="text/javascript">
            $(function() {
                ...
                $('.headerRow.totalRow th').first().html(
                        ...
                        '2014 Without Alternatives:<br/><br/>' +
                        '2013 Point Totals:<br/><br/>' +
                        '2012 Point Totals:'
                );
