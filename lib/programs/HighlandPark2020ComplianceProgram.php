...
class HighlandParkMyAndSpousePointsView extends ComplianceView
{
    ...
    public function getStatus(User $user)
    {
        ...
        foreach($this->groupsComplianceRequired as $group) {
            $groupStatus = $myStatus->getComplianceViewGroupStatus($group);

            $pointsToRemove += $this->countPointsFromGroupsRequired ? 0 : $groupStatus->getPoints();
        }

        $points = $myStatus->getPoints() - $pointsToRemove;
        ...
        if($spouseUser) {
            ...
            $pointsToRemove = 0;
            foreach($this->groupsComplianceRequired as $group) {
                $groupStatus = $spouseStatus->getComplianceViewGroupStatus($group);

                $pointsToRemove += $this->countPointsFromGroupsRequired ? 0 : $groupStatus->getPoints();
            }

            $spousePoints = $spouseStatus->getPoints() - $pointsToRemove;
...
class HighlandPark2020HomePageComplianceProgram extends ComplianceProgram
{
    const SCHEDULE_BUTTON_OVERRIDE_TYPE = 'highland_park_schedule_button_override_2020';
    ...
    public function loadGroups()
    {
        ...
        $appointmentView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) use($appointmentView) {
            if($user->getNewestDataRecord(HighlandPark2020HomePageComplianceProgram::SCHEDULE_BUTTON_OVERRIDE_TYPE, true)->clicked) {
        ...
        $winView->setPostEvaluateCallback(function(ComplianceViewStatus $status, User $user) {
            ...
            if($activeProgramRecord->id == 1469) {
                if($programStatus->getComplianceViewStatus('complete_screening')->getStatus() == ComplianceStatus::COMPLIANT
                    ...
                    && $programStatus->getComplianceViewStatus('spouse')->getPoints() >= 150) {
                    $status->setStatus(ComplianceStatus::COMPLIANT);
                }
            }
...
class HighlandPark2020ComplianceProgram extends ComplianceProgram
{
    public function getTrack(User $user)
    {
        if($this->lastTrack && $this->lastTrack['user_id'] == $user->id) {
            ...
            $track = $user->getGroupValueFromTypeName('Advocate Track 2020', '0-1 Risk Factors');
    ...
    {
        return new HighlandPark2020ComplianceProgramReportPrinter();
    }

    private $lastTrack = null;
}

class HighlandPark2020ComplianceProgramReportPrinter extends BasicComplianceProgramReportPrinter
{
    public function printReport(ComplianceProgramStatus $status)
    {
        $this->setShowLegend(true);
        $this->pageHeading = '2020 WIN (Wellness Initiative Program)';
        ...
        <script type="text/javascript">
            $(function() {
                ...
                $('tr.view-mine').prev('tr.headerRow').find('td').last().html(
                    'Minimum Points Needed for Incentive By 10/31/2020'
                );
                ...
                $('.view-coaching').children(':eq(0)').html('<strong>C</strong>. Health Coaching: Complete 3 Sessions (Must begin by September 28, 2020)');
                ...
                $('.view-coaching').children(':eq(0)').html('<strong>C</strong>. Health Coaching: Complete 6 Sessions (Must begin by August 10, 2020)');
                $('.view-coaching').children('.result').html('<?php echo $coachingStatus->getPoints() ?>');
                $('.view-coaching').children('.links').html('To schedule an appointment, send email to <a href="mailto:AMG-AAWCallCenter@advocatehealth.com">AMG-AAWCallCenter@advocatehealth.com</a> OR Call the Advocate Aurora Contact Center at 800.761.5856');
                <?php else : ?>
