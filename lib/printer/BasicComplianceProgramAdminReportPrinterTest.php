...
class BasicComplianceProgramAdminReportPrinterTest implements ComplianceProgramAdminReportPrinter
{
    /**
    ...
    public function setHideUserData($bool) {
        $this->hideUserData = $bool;
    }
    ...
    public function setShowUtilization($bool)
    {
        $this->showUtilization = $bool;
    }
    ...
    public function printAdminReport(ComplianceProgramReport $report, $output)
    {
            ...
            foreach($report as $status) {
                ...
                foreach($status->getComplianceViewGroupStatuses() as $groupStatus) {
                    ...
                    if(
                        ...
                        if ($groupStatus->getPoints() > 0) {
                            $totals[$groupKey]['utilization']+= $groupStatus->getPoints();
                        } else {
                            $totals[$groupKey]['utilization']++;
                        }
                    }
                    ...
                    foreach($groupStatus->getComplianceViewStatuses() as $viewStatus) {
                        ...
                        if(
                            ...
                            if ($viewStatus->getPoints() > 0) {
                                $totals[$viewKey]['utilization']+= $viewStatus->getPoints();
                                $utilizationNumber += $viewStatus->getPoints();
                            } else {
                                $totals[$viewKey]['utilization']++;
                                $utilizationNumber++;
                            }
                        }
        ...
        if($this->showUtilization) {
            ...
            $totals['Compliance Program']['utilization'] = $utilizationNumber;

            fputcsv($output, array('Name', 'Total Utilization', 'Total Unique Participants'));

            foreach($totals as $name => $total) {
                $utilizationPercent = round($total['utilization'] / $i * 100, 2);

                fputcsv($output, array(
                    $name,
                    "{$total['utilization']} ($utilizationPercent%)",
                    "{$total['participating']}",
                ));
            }
        }
